 ...
 */
export function jobsRegistry(ctx: JobsRegistryHost): JobsRegistry | undefined {
  const service = ctx.get('jobs')
  if (typeof service !== 'object' || service === null) return undefined
  const registry = service as Partial<JobsRegistry>
  return typeof registry.list === 'function' && typeof registry.onJobsChanged === 'function'
    ? (registry as JobsRegistry)
    : undefined
}
 ...
 */
export function isLiveJob(row: JobRow): boolean {
 ...
 */
export function jobCounts(rows: readonly JobRow[]): JobCounts {
  let live = 0
 ...
 */
export function sortJobRows(rows: readonly JobRow[]): JobRow[] {
  return [...rows].sort((left, right) => {
    ...
    // A settled job with no `finishedAt` cannot happen through the registry,
    // which stamps it on settlement; falling back to `startedAt` keeps such a
    // row in a defined place rather than at an accidental end of the list.
    const endLeft = left.finishedAt ?? left.startedAt
    const endRight = right.finishedAt ?? right.startedAt
    return endRight - endLeft || left.id.localeCompare(right.id)
  })
}
 ...
 */
export function jobElapsed(row: JobRow, now: number): number {
