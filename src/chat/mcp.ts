...
import { formatDiagnosticCount } from '../components/dialogs.ts'
 ...
 *
 * The example is the client's own README row, kept exact: a user who copies it
 * gets a working stdio server, and a wrong field name here would cost more than
 * the panel saves. Every line is plain text — the panel dims the block whole.
 */
export const MCP_NOT_MOUNTED_LINES: readonly string[] = [
  'No MCP tools are registered for this agent.',
  '',
  'MCP servers reach the model through @deepseek-ai/dsh-mcp-client, one plugin',
  ...
  '',
  'transport is "stdio" (command, args, env, cwd) or "streamable-http" (url,',
...
export function renderMcpPanel(names: readonly string[], palette: Palette): readonly string[] {
  ...
  if (servers.length === 0) return MCP_NOT_MOUNTED_LINES.map(line => palette.dim(line))
  const total = servers.reduce((count, server) => count + server.tools.length, 0)
  return [
    palette.dim(`${formatDiagnosticCount(servers.length, 'server')} · ${formatDiagnosticCount(total, 'tool')}`),
    ...servers.flatMap(server => [
      '',
      `${palette.bold(palette.accent(displayInlineText(server.server)))} ${palette.dim(`(${formatDiagnosticCount(server.tools.length, 'tool')})`)}`,
