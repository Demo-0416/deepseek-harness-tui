 ...
 */
export function isInvalidTitleError(error: unknown): boolean {
  return typeof error === 'object' && error !== null
    && (error as { name?: unknown }).name === 'SessionTitleInvalidError'
}
...
export async function runRenameCommand(
  ...
  if (titles === undefined) return { kind: 'error', text: t('rename.unavailable') }
  const typed = rawInput.trim()

  if (typed !== '') {
    try {
      const accepted = titles.rename(session, typed)
      return { kind: 'success', text: t('rename.done', { title: displayInlineText(accepted.title) }) }
    } catch (error: unknown) {
      if (isInvalidTitleError(error)) return { kind: 'error', text: t('rename.invalid') }
      return { kind: 'error', text: t('rename.failed', { error: displayInlineText(errorChain(error)) }) }
    }
  }
