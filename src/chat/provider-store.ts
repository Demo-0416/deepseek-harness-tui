...
export function apiKeyRefusal(key: string): string | undefined {
  if (key === '') return 'Enter a key, or press Esc to cancel.'
  if (ILLEGAL_KEY_PATTERN.test(key)) return 'That key contains whitespace or control characters; paste it again.'
  return undefined
}
...
export function routeKeyRefusal(route: string): string | undefined {
  if (route === '') return 'Enter a name, or press Esc to cancel.'
  if (!ROUTE_KEY_PATTERN.test(route)) {
    return 'A provider name starts with a letter or digit and holds only letters, digits, dot, dash, and underscore.'
  }
  return undefined
}
...
export function credentialRefRefusal(ref: string): string | undefined {
  if (ref === '') return 'Enter a variable name, or press Esc to cancel.'
  if (!CREDENTIAL_REF_PATTERN.test(ref)) {
    return 'An environment variable name starts with a letter or underscore and holds only letters, digits, and underscore.'
  }
  return undefined
}
