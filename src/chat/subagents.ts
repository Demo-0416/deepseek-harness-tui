...
export interface SubagentDirectory {
  ...
  listDescendants(
    ...
    signal?: AbortSignal,
  ): Promise<readonly SubagentDescendant[]>
}
 ...
 */
export function subagentDirectory(ctx: SubagentDirectoryHost): SubagentDirectory | undefined {
  const service = ctx.get('subagents')
  if (typeof service !== 'object' || service === null) return undefined
  const directory = service as Partial<SubagentDirectory>
  return typeof directory.listDescendants === 'function'
    ? (directory as SubagentDirectory)
    : undefined
}
 ...
 */
export function subagentCounts(entries: readonly SubagentDescendant[]): SubagentCounts {
  let running = 0
  for (const entry of entries) {
    if (entry.kind === 'child' && entry.activity === 'running') running += 1
  }
 ...
 */
export function subagentName(entry: SubagentDescendant): string {
  return entry.kind === 'child' && entry.label !== undefined && entry.label !== ''
    ? entry.label
    : entry.id
}
