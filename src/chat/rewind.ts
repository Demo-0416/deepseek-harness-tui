 ...
 */
export function rewindTargets(events: readonly SessionEvent[]): RewindTarget[] {
  ...
  for (const event of events) {
    ...
    if (text === '') continue
    targets.push({ seq: event.seq, time: event.time, text })
  }
  return targets
}
 ...
 */
export function hasRewindTarget(events: readonly SessionEvent[]): boolean {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index]
    if (event?.type !== 'user/message') continue
    const source: { kind?: unknown } = event.data.source
    if (source.kind === 'user' && contentText(event.data.content).trim() !== '') return true
  }
  return false
}
