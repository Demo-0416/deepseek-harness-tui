...
/** Row labels for the roles that do not carry one of their own. */
const ROLE_LABELS: Record<TranscriptEntryRole, string> = {
  user: 'You',
  assistant: 'Assistant',
  tool: 'Tool',
  notice: 'Notice',
...
function entryOf(node: ChatNode): Pick<TranscriptEntry, 'role' | 'label' | 'text'> | undefined {
  switch (node.kind) {
    case 'user-message':
      ...
      return node.withdrawn === true ? undefined : { role: 'user', label: ROLE_LABELS.user, text: node.text }
    case 'assistant':
      ...
      return { role: 'assistant', label: ROLE_LABELS.assistant, text: node.text }
    case 'tool-call': {
    ...
    case 'notice':
      return { role: 'notice', label: ROLE_LABELS.notice, text: node.text }
    case 'context':
      return { role: 'context', label: node.label, text: node.text }
    case 'reference':
      return { role: 'reference', label: ROLE_LABELS.reference, text: node.labels.join('\n') }
    case 'compaction':
      return node.landed ? { role: 'compaction', label: ROLE_LABELS.compaction, text: node.summary } : undefined
    default:
