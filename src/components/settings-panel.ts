...
/** The key hints the panel ends with. */
export const SETTINGS_PANEL_HINT = '↑↓ move · enter change · esc close'
...
export class SettingsPanel implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    return [
      ...
      ...shown.map(line => ` ${line}`),
      ` ${truncateToWidth(this.palette.dim(`${SETTINGS_PANEL_HINT}${position}`), contentWidth, '')}`,
    ]
  }
}
