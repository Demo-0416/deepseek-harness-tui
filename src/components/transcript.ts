...
export class HeaderComponent implements Component {
  ...
  private resumedLine(): string | undefined {
    ...
    return t('banner.resumed', { id: displayText(this.info.resumed) })
...
export function planModeRow(
  ...
  return modeRow(
    palette,
    brandSchemeColors(scheme).planMode,
    `${PLAN_MODE_ICON} ${t(pending ? 'transcript.planModeBadgePending' : 'transcript.planModeBadge')}`,
    hint,
  )
}
...
export function autoAcceptRow(
  ...
  return modeRow(
    palette,
    brandSchemeColors(scheme).autoAccept,
...
export class CollapsedGroupComponent extends CachedCardComponent {
  ...
  protected renderLines(width: number): string[] {
    ...
    const bullet = group.active
      ? this.palette.bold(accent(this.palette, group.failed ? BRAND_COLORS.error : BRAND_COLORS.brand, TOOL_BULLET))
      : group.failed ? accent(this.palette, BRAND_COLORS.error, TOOL_BULLET) : ' '
