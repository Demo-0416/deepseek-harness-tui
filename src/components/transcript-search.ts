...
/** Shown when the session has produced nothing readable yet. */
export const TRANSCRIPT_SEARCH_EMPTY = 'This session has no messages to search yet.'

/** Shown when the query matches nothing; the messages themselves are still there. */
export const TRANSCRIPT_SEARCH_NO_MATCH = 'No message matches this search.'

/** Terminal rows the panel spends on its own chrome: blank, title, query, count, footer. */
const PANEL_CHROME_ROWS = 5

/** The key hints the list ends with, before its position readout. */
const LIST_HINT = 'type to search · ↑↓ move · enter open · esc close'

/** The key hints one message's page ends with, before its position readout. */
const DETAIL_HINT = '↑↓ scroll · PgUp/PgDn page · esc back'
...
export class TranscriptSearchPanel implements Component, Focusable {
  ...
  private listBody(visible: readonly TranscriptMatch[], width: number): { rows: string[]; selectedRow: number } {
    if (visible.length === 0) return { rows: [this.palette.dim(TRANSCRIPT_SEARCH_NO_MATCH)], selectedRow: 0 }
  ...
  private frame(rows: readonly string[], offset: number, keep?: number): {
    ...
    const position = rows.length > viewport
      ? `  ·  ${String(next + 1)}–${String(next + shown.length)} of ${String(rows.length)}`
      : ''
  ...
  render(width: number): string[] {
    ...
    if (this.entries.length === 0) {
      return [
        '',
        title,
        ...wrapTextWithAnsi(this.palette.dim(TRANSCRIPT_SEARCH_EMPTY), contentWidth).map(line => ` ${line}`),
  ...
  private renderList(contentWidth: number, title: string): string[] {
    ...
    const queryLine = truncateToWidth(
      `${this.palette.dim('search')} ${this.query.render(Math.max(1, contentWidth - 7)).join('')}`,
      contentWidth,
      '',
    )
    const count = `${String(visible.length)}/${String(this.entries.length)} messages`
    ...
    return [
      ...
      ` ${truncateToWidth(this.palette.dim(`${LIST_HINT}${framed.position}`), contentWidth, '')}`,
    ]
  }
  ...
  private renderDetail(entry: TranscriptEntry, contentWidth: number): string[] {
    ...
    const subtitle = query === ''
      ? 'the whole message'
      : `hits for "${displayInlineText(query)}" are highlighted`
    return [
      '',
      ...
      '',
      ...framed.shown.map(line => ` ${line}`),
      ` ${truncateToWidth(this.palette.dim(`${DETAIL_HINT}${framed.position}`), contentWidth, '')}`,
    ]
  }
}
