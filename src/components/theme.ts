...
/** One line per `/theme` value, shown beside it in the selector and its completions. */
export const THEME_PREFERENCE_DESCRIPTIONS: Readonly<Record<ThemePreferenceId, string>> = {
  auto: 'Follow the color scheme the terminal reports',
  light: 'Always paint the light-background palette',
  dark: 'Always paint the dark-background palette',
  'no-color': 'Emit no color at all',
}
