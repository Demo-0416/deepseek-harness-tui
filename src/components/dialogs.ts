 ...
 */
export const CUSTOM_ANSWER_LABEL = t('dialog.question.customAnswerLabel', undefined, 'en')
...
export class QuestionDialog implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    const progress = this.total > 1
      ? this.palette.dim(`  ${t('dialog.question.progress', {
  ...
  private renderInputBlock(innerWidth: number): string[] {
    ...
    // Read per frame, so `/lang` moves the placeholder the same way it moves
    // the option list around it. Beside checkboxes the row is one of several
    // things being picked rather than the whole answer, so it drops the stop.
    const label = t('dialog.question.customAnswerLabel')
    const value = this.input.getValue()
    const body = value === ''
      ? this.palette.dim(this.question.multiSelect ? label.replace(/[.。]$/u, '') : label)
