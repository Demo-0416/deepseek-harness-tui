 ...
 * each node index in it; {@link collapsedSummary} words the row.
 * @module dsh-tui/core/collapse
 */

import type { ChatNode, ToolCallNode } from './types.ts'

/** What a collapsed row calls each kind of read-only operation. */
export const COLLAPSE_TEXT = {
  /** `Searched for N patterns` / `Searching for N patterns…`. */
  search: { active: 'searching for', settled: 'searched for', one: 'pattern', many: 'patterns' },
  /** `Read N files` / `Reading N files…`. */
  read: { active: 'reading', settled: 'read', one: 'file', many: 'files' },
  /** `Listed N directories` / `Listing N directories…`. */
  list: { active: 'listing', settled: 'listed', one: 'directory', many: 'directories' },
  /** `Queried <server> N times`; the count is dropped for a single call. */
  mcp: { active: 'querying', settled: 'queried', times: 'times' },
  /** Trailing ellipsis on a group that is still running. */
  ellipsis: '…',
  /** The dim hint that names the key which opens the group. */
...
/** `1 file` / `2 files`, with the noun the count needs. */
function counted(count: number, one: string, many: string): string {
  return `${count} ${count === 1 ? one : many}`
}

/** Capitalize a fragment when it opens the sentence, leave it alone otherwise. */
function opener(verb: string, first: boolean): string {
  return first ? verb.charAt(0).toUpperCase() + verb.slice(1) : verb
}
 ...
 * The first fragment opens with a capital, later ones do not, and each noun
 * agrees with its own count.
 * @param group - The planned group.
 * @returns The row's text, without the `(ctrl+o to expand)` hint.
 */
export function collapsedSummary(group: CollapsedGroup): string {
  const parts: string[] = []
  const tense = (text: { active: string; settled: string }): string =>
    opener(group.active ? text.active : text.settled, parts.length === 0)
  if (group.searchCount > 0) {
    parts.push(`${tense(COLLAPSE_TEXT.search)} ${counted(
      group.searchCount,
      COLLAPSE_TEXT.search.one,
      COLLAPSE_TEXT.search.many,
    )}`)
  }
  if (group.readCount > 0) {
    parts.push(`${tense(COLLAPSE_TEXT.read)} ${counted(
      group.readCount,
      COLLAPSE_TEXT.read.one,
      COLLAPSE_TEXT.read.many,
    )}`)
  }
  if (group.listCount > 0) {
    parts.push(`${tense(COLLAPSE_TEXT.list)} ${counted(
      group.listCount,
      COLLAPSE_TEXT.list.one,
      COLLAPSE_TEXT.list.many,
    )}`)
  }
  if (group.mcpCallCount > 0) {
    ...
    // group actually queried more than once.
    const times = group.mcpCallCount > 1 ? ` ${group.mcpCallCount} ${COLLAPSE_TEXT.mcp.times}` : ''
    parts.push(`${tense(COLLAPSE_TEXT.mcp)} ${server}${times}`)
  }
  const text = parts.join(', ')
  return group.active ? `${text}${COLLAPSE_TEXT.ellipsis}` : text
}
