 ...
 */
export function workflowRunStatus(node: WorkflowRunNode): WorkflowRunStatus {
  switch (node.stopReason) {
    ...
    case 'error':
      return 'failed'
    default:
      return node.endedAt === undefined ? 'running' : 'interrupted'
  }
}
 ...
 * @returns the derived status.
 */
export function workflowMemberStatus(node: WorkflowRunNode, member: WorkflowMemberEntry): WorkflowRunStatus {
 ...
 */
export function groupWorkflowPhases(node: WorkflowRunNode): WorkflowPhaseGroup[] {
  // A Map keyed by the phase string keeps insertion order; the absent phase
  // gets its own bucket beside it because `undefined` is not a valid key here.
  const named = new Map<string, WorkflowMemberEntry[]>()
  const missing: WorkflowMemberEntry[] = []
  const order: (string | undefined)[] = []
  for (const member of node.members) {
    if (member.phase === undefined) {
      if (missing.length === 0) order.push(undefined)
      missing.push(member)
      continue
    }
  ...
  return order.map((phase) => ({
    phase,
    members: phase === undefined ? missing : named.get(phase) ?? [],
  }))
}
...
export function workflowStatusCounts(
  ...
  members: readonly WorkflowMemberEntry[],
): WorkflowStatusCounts {
  const counts = { running: 0, completed: 0, failed: 0, cancelled: 0, interrupted: 0 }
