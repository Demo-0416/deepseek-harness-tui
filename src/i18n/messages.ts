...
export const EN_MESSAGES = {
  ...
  // Shift+Tab. Each line says what the mode DOES rather than naming it twice:
  // the badge above the prompt already carries the name, and this row is the
  // one chance to tell a user what they just switched on.
  ...
  // each of the three outcomes — accepted, unchecked, refused — is its own row
  // rather than a sentence assembled from a boolean.
  ...
  // The completed write, reported in four sentences: what happened, where the
  // secret went, whether it was proven, and when it starts working.
  'login.saved.added': 'Added provider',
  'login.saved.signedIn': 'Signed in to',
  'login.saved.summary': '{what} {provider} with key {key}.',
  'login.saved.credential':
    'The key is stored in the credential store as {ref}; settings record the variable name only.',
  'login.saved.verified': 'The endpoint accepted it.',
  'login.saved.unverified': 'It was not checked against the endpoint.',
  // Read from the namespace owner's declared effect timing, never guessed.
  'login.effect.live': 'It is live now — /model lists its models.',
  'login.effect.restart': 'Restart dsh for it to take effect.',
  'login.effect.unknown':
    'This deployment does not say whether that applies without a restart; if /model does not list it, restart dsh.',
  'login.noProviders':
  ...
  // Refusals the key field answers with; stated where the value is judged.
  ...
  'provider.protocol.lead':
    'Only OpenAI-shaped endpoints can be interrogated for their models; the rest are saved unchecked.',
  ...
  'provider.refuse.routeShape':
    'A provider name starts with a letter or digit and holds only letters, digits, dot, dash, and underscore.',
  'provider.refuse.credentialEmpty': 'Enter a variable name, or press Esc to cancel.',
  'provider.refuse.credentialShape':
    'An environment variable name starts with a letter or underscore and holds only letters, digits, and underscore.',
