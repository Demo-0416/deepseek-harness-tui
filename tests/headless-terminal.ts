/**
 * Headless ANSI terminal used by TUI tests. It consumes the same byte stream a
 * real terminal would, so assertions read rendered cells rather than raw escape
 * codes, and exposes pi-tui's synchronized-update frames as an awaitable boundary.
 *
 * Ported from the upstream DeepSeek Harness TUI test suite; the pi-tui
 * `Terminal` surface it implements is the 0.84.1 one.
 * @module dsh-tui/tests/headless-terminal
 */
...
// `@xterm/headless` ships a UMD bundle, so Node's ESM loader cannot detect its
// named exports; `import { Terminal }` throws at load time under `node --test`.
// The upstream suite runs on Vitest, whose CJS interop hides this.
const { Terminal: XtermTerminalImpl } = createRequire(import.meta.url)('@xterm/headless') as typeof import('@xterm/headless')

/** Synchronized-update end sequence pi-tui writes once per completed frame. */
...
export class HeadlessTerminal implements Terminal {
  ...
  constructor(columns = 80, rows = 24) {
    this.emulator = new XtermTerminalImpl({
      cols: columns,
      rows,
      scrollback: 1_000,
  ...
  /** Feed input bytes to the mounted TUI exactly as a keyboard would. */
  ...
  /** Read the rendered viewport as plain text rows, styles discarded. */
  text(options: TerminalSnapshotOptions = {}): string {
    ...
    for (let index = 0; index < rowCount; index++) {
      rows.push(this.emulator.buffer.active.getLine(firstRow + index)?.translateToString(true) ?? '')
    }
    return rows.join('\n')
  }
