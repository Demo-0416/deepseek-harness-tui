...
import { isClipboardExportTarget, renderSessionMarkdown } from '../../src/chat/export.ts'
...
describe('/export clipboard', { skip: skipWithoutEntry }, () => {
  it('puts the session on the clipboard as Markdown', async () => {
    ...
    try {
      ...
      assert.match(result?.text ?? '', /^Exported \d+ messages? as Markdown\. Sent to clipboard via OSC 52\.$/u)
