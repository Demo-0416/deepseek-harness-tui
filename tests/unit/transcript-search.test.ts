...
describe('TranscriptSearchPanel', () => {
  ...
  it('renders its own chrome in the active locale', () => {
    // The entries carry the labels of the locale they were folded under, which
    // is the locale the panel was opened in; everything the panel draws itself
    // is read at render time, like every other surface here.
    setLocale('zh')
    try {
      const frame = rows(fixture())
      assert.match(frame[3] ?? '', /^4\/4 条消息/u)
      assert.match(frame[4] ?? '', /^→ 你\s+Where does the Banner sweep start\?/u)
      assert.match(frame.at(-1) ?? '', /输入以搜索 · ↑↓ 移动 · enter 展开 · esc 关闭/u)
      const empty = new TranscriptSearchPanel([], '', () => 12, palette, () => {})
      assert.deepEqual(rows(empty), ['', '/search', '本会话还没有可搜索的消息。', 'esc 关闭'])
    } finally {
      setLocale('en')
    }
  })
