		...
		for (let index = this.hiddenBefore; index < nodes.length; index += 1) {
			...
			switch (node.kind) {
				...
				case "compaction": {
					...
					seen.add(markerKey);
					const hint = node.summary === "" || opened ? "" : ` ${t("collapse.expandHint", { key: this.deps.expandKey().toLowerCase() })}`;
...
*/
function nextContextAnnouncement(tracker, level) {
	if (pressureRank(level) <= pressureRank(tracker.announced)) {
...
async function runCompactCommand(deps, rawInput, signal, commandId) {
	...
	if (deps.agent.status !== "idle") return {
		kind: "error",
		text: t("compact.busy", { status: deps.agent.status })
	};
	try {
	...
	*/
	const exportSessionMarkdown = async (signal) => {
		...
		try {
			...
			return {
				kind: "success",
				text: plural(entries.length, "export.clipboard.done", { detail: clipboardConfirmation(path, markdown.length) })
			};
		} catch (error) {
			return {
				kind: "error",
