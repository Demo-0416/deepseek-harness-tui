...
import { accessSync, closeSync, constants, existsSync, mkdirSync, openSync, readFileSync, readSync, statSync, writeFileSync } from "node:fs";
import { appendFile, lstat, mkdir, readFile, readdir, rm, stat, unlink, writeFile } from "node:fs/promises";
...
*/
const APPROVAL_RULES_FLUSH_TIMEOUT_MS = 1e3;
...
function serializeApprovalRule(rule) {
	const escaped = rule.content === void 0 ? rule.tool : `${rule.tool}(${rule.content.replaceAll("\\", "\\\\").replaceAll("(", "\\(").replaceAll(")", "\\)")})`;
	return rule.access === void 0 ? escaped : `${escaped} [${rule.access}]`;
}
/**
...
*/
function escalationAccess(reason) {
...
*/
function isInsideProject(cwd, projectCwd) {
...
*/
function commandMatchesRuleContent(command, content) {
	const isPrefix = content.endsWith(PREFIX_SUFFIX);
	const wanted = commandTokens(isPrefix ? content.slice(0, -2) : content);
...
*/
function suggestCommandPrefix(command) {
	const line = command.trim();
	if (line === "" || isCompoundCommand(line)) return void 0;
	const tokens = line.split(/\s+/u);
	const head = tokens[0] ?? "";
	if (head.includes("=") || WRAPPER_COMMANDS.has(head)) return void 0;
	const second = tokens[1];
	if (second !== void 0 && SUBCOMMAND_WORD.test(second)) return `${head} ${second}${PREFIX_SUFFIX}`;
...
*/
function readProjectRules(document, cwd) {
	...
	return allow.filter((rule) => typeof rule === "string" && rule.trim() !== "");
}
/**
...
*/
function openApprovalRules(options) {
	const path = options.path ?? dshHomePath("approvals.json");
	const displayPath = options.path ?? `${dshHomeDisplay(resolveDshHome())}/approvals.json`;
	...
	*/
	const persist = async (serialized) => {
		...
		await withFileLock(path, async () => {
			...
			try {
				...
				if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) document = parsed;
			} catch (_missingOrCorrupt) {}
			...
			const projects = typeof document.projects === "object" && document.projects !== null ? { ...document.projects } : {};
	...
	return {
		displayPath,
		matchesTool(toolName, context = {}) {
			return grants.some((grant) => grant.content === void 0 && grant.tool === toolName && grant.access === context.access);
		},
		matchesCommand(toolName, command, context = {}) {
			const trimmed = command.trim();
			if (trimmed === "" || isCompoundCommand(trimmed)) return false;
			if (context.cwd !== void 0 && !isInsideProject(context.cwd, options.cwd)) return false;
			return grants.some((grant) => grant.tool === toolName && grant.content !== void 0 && grant.access === context.access && commandMatchesRuleContent(trimmed, grant.content));
		},
		allow(rule) {
...
const EN_MESSAGES = {
	...
	"hotkeys.externalEditor": "{key} edit the draft in $EDITOR — $VISUAL, then $EDITOR, then a terminal editor on PATH",
	...
	"hotkeys.approval": "In an approval: ↑/↓ move • a digit answers the row it numbers (a fifth row appears when the answer can be remembered) • Enter confirm • Esc deny",
	...
	"doctor.model.noProviderAdvice": "the profile mounts no adapter row, or none of them activated; check the bundle, or give a provider a key with /login",
	...
	"doctor.model.failedAdvice": "the adapter is registered but rejected the lookup; check the base URL, and run /login if the key is the suspect",
	...
	"approval.allowProject": "Yes, and don't ask again for {tool} in this project",
	"approval.allowProjectAccess": "Yes, and don't ask again for {tool} at {access} access in this project",
	"approval.allowPrefix": "Yes, and don't ask again for commands matching: {prefix}",
	"approval.allowPrefixAccess": "Yes, and don't ask again for commands matching {prefix} at {access} access",
	"approval.commandCwd": "Runs in {path}, outside this project",
	...
	"approval.prefixPrompt": "Edit the rule to remember (leave empty to allow only once):",
	"approval.prefixHint": "Enter save the rule and allow • Esc back to the answers",
	"approval.grantedSession": "Allowing {tool} for the rest of this session in this terminal. Restarting or resuming asks again.",
	"approval.grantedProject": "Allowing {tool} in this project from now on. Stored in {path}; edit that file to revoke.",
	"approval.grantedProjectAccess": "Allowing {tool} at {access} access in this project from now on. Stored in {path}; edit that file to revoke.",
	"approval.grantedPrefix": "Allowing commands matching {rule} in this project from now on. Stored in {path}; edit that file to revoke.",
	"approval.diffClipped": "… ({count} more diff lines • rest not shown)",
	"approval.diffMoreFiles": "… and {count} more files this call would change",
	"approval.diffNoChanges": "no changes",
	"approval.diffApproximate": "exact line diff omitted: more than {limit} changed lines",
	...
	"notice.promptTruncated": "That prompt was {original} characters; the limit is {limit}, so {removed} characters were dropped from its middle before it was sent. Attach long files with @path instead of pasting them, or raise maxPromptChars.",
	"notice.externalEditorUnset": "No external editor found: set $EDITOR or $VISUAL, or configure externalEditor.",
	...
	"notice.contextLow": "Context low — {remaining}% of the window left ({used} / {capacity} tokens). {action}",
	"notice.contextCritical": "Context nearly full — only {remaining}% of the window left ({used} / {capacity} tokens). {action}",
	"notice.contextCompactAction": "Run /compact to summarize the history and keep going.",
	"notice.contextCompactActionAfterTurn": "When this turn ends, run /compact to summarize the history and keep going.",
	"notice.contextNoCompactAction": "This session composes no compaction service; start a fresh session with /new before the window fills.",
	"compact.usage": "/compact takes no arguments. The summary's instructions come from the compaction backend, so there is nothing here to steer.",
	"compact.unavailable": "This session's agent preset mounts no compaction service, so there is nothing to compact with.",
	...
	"compact.nothing": "Nothing to compact yet: this session has no history the model can safely stop seeing.",
	...
	"compact.error.busy": "Compaction is unavailable: this process already has one running, or the agent is not idle.",
	"compact.error.changed": "The history selected for compaction changed before it could be replaced. The conversation is unchanged; the attempt is in the session log.",
	"compact.error.summary": "Compaction could not produce a useful summary. The conversation is unchanged; the attempt is in the session log.",
	"compact.error.commit": "Compaction did not finish cleanly; some session history may have changed. Check the session before retrying.",
	...
	"export.clipboard.truncated.other": "Exported {count} entries as Markdown, truncated to {limit} characters. {detail}",
	...
	"login.select.title": "Sign in to a provider",
	"login.select.lead": "Configured routes take a new key; the rest are written into settings when you sign in.",
	"login.group.configured": "Configured",
	"login.group.available": "Available to configure",
	"login.roster.noKey": "no key stored",
	"login.roster.keyIn": "key in {env}",
	"login.key.title": "Sign in to {provider}",
	"login.key.catalogEndpoint": "This provider's endpoint comes from the built-in catalog, so the key cannot be checked from here.",
	"login.key.endpoint": "Endpoint: {url}",
	"login.key.storage": "The key is stored in the credential store, never in settings.",
	"login.key.prompt": "Paste the API key. It is never echoed.",
	"login.key.rejected": "The endpoint rejected that key: {message}. Nothing was saved.",
	"login.status.checking": "Checking the key against the endpoint…",
	"login.status.saving": "Saving…",
	"login.unverified.title": "Key not verified",
	"login.unverified.reassure": "Storing it is safe; it simply has not been proven to work yet.",
	"login.unverified.save": "Save the key anyway",
	"login.unverified.saveDetail": "checked on first use",
	"login.unverified.cancel": "Cancel",
	"login.unverified.cancelDetail": "nothing is written",
	"login.saved.added": "Added provider",
	"login.saved.signedIn": "Signed in to",
	"login.saved.summary": "{what} {provider} with key {key}.",
	"login.saved.credential": "The key is stored in the credential store as {ref}; settings record the variable name only.",
	"login.saved.verified": "The endpoint accepted it.",
	"login.saved.unverified": "It was not checked against the endpoint.",
	"login.effect.live": "It is live now — /model lists its models.",
	"login.effect.restart": "Restart dsh for it to take effect.",
	"login.effect.unknown": "This deployment does not say whether that applies without a restart; if /model does not list it, restart dsh.",
	"login.noProviders": "No providers are configured and none are offered by a registered adapter. Use /provider add to describe one.",
	"login.noStore": "This deployment has no writable settings or credential store, so a key cannot be saved here.",
	"login.unknownProvider": "Unknown provider: {name}. Run /login to list them.",
	"login.error.save": "Could not save: {error}",
	"login.error.start": "Could not start sign-in: {error}",
	"login.refuse.keyEmpty": "Enter a key, or press Esc to cancel.",
	"login.refuse.keyIllegal": "That key contains whitespace or control characters; paste it again.",
	"provider.usage": "Usage: /provider [add]",
	"provider.arg.add": "Describe an endpoint no adapter ships",
	"provider.noStore": "This deployment has no writable settings or credential store, so a provider cannot be added here.",
	...
	"provider.add.lead": "Written to the {ns} settings section.",
	"provider.add.prompt": "Route name, as /model will spell it (for example acme).",
	"provider.add.taken": "{value} is already configured; /login gives it a new key.",
	"provider.endpoint.title": "Endpoint",
	"provider.endpoint.lead": "The base URL the adapter appends its paths to.",
	"provider.endpoint.prompt": "Base URL, for example https://api.example.com/v1",
	"provider.endpoint.refuseEmpty": "Enter a URL, or press Esc to cancel.",
	"provider.endpoint.refuseShape": "Enter an http:// or https:// URL.",
	"provider.protocol.title": "Wire protocol",
	"provider.protocol.lead": "Only OpenAI-shaped endpoints can be interrogated for their models; the rest are saved unchecked.",
	"provider.protocol.noListing": "no model listing",
	"provider.probe.unsupported": "{api} endpoints do not list their models, so the key cannot be checked from here.",
	"provider.probe.silentFailure": "the endpoint failed without saying why",
	"provider.displayName.title": "Display name",
	"provider.displayName.lead": "Shown in pickers. Leave empty to use the route name.",
	"provider.displayName.prompt": "Display name (optional).",
	"provider.credential.title": "Credential variable",
	"provider.credential.lead": "Settings record this name; the secret itself lives in the credential store.",
	"provider.credential.prompt": "Environment variable name.",
	"provider.key.title": "Key for {route}",
	"provider.key.storedAs": "Stored in the credential store as {ref}.",
	...
	"provider.refuse.routeShape": "A provider name starts with a letter or digit and holds only letters, digits, dot, dash, and underscore.",
	"provider.refuse.credentialEmpty": "Enter a variable name, or press Esc to cancel.",
	"provider.refuse.credentialShape": "An environment variable name starts with a letter or underscore and holds only letters, digits, and underscore.",
	"dialog.provider.empty": "Nothing to choose from.",
	"dialog.provider.hintBusy": "Esc cancel",
	"dialog.provider.hintText": "Enter continue · Ctrl+U clear · Esc cancel",
	"dialog.provider.hintChecklist": "↑↓ move · Space tick · Enter continue · Esc cancel",
	"dialog.provider.hintSelect": "↑↓ move · Enter select · Esc cancel"
};
/**
* The Chinese table.
*
...
const ZH_MESSAGES = {
	...
	"hotkeys.externalEditor": "{key} 用 $EDITOR 编辑草稿——先看 $VISUAL，再看 $EDITOR，最后在 PATH 上找终端编辑器",
	...
	"hotkeys.approval": "授权框内：↑/↓ 移动 • 数字键直接作答（可记住的授权会多出第 5 行） • Enter 确认 • Esc 拒绝",
	...
	"approval.reject": "拒绝",
	"approval.allowProject": "同意，本项目内不再询问 {tool}",
	"approval.allowProjectAccess": "同意，本项目内不再询问 {tool} 的 {access} 权限请求",
	"approval.allowPrefix": "同意，且不再询问匹配该规则的命令：{prefix}",
	"approval.allowPrefixAccess": "同意，且不再询问以 {access} 权限运行、匹配 {prefix} 的命令",
	"approval.commandCwd": "将在 {path} 运行（不在本项目目录内）",
	"approval.feedbackPrompt": "告诉 agent 该怎么做：",
	"approval.feedbackHint": "Enter 带着这段说明拒绝 • Esc 返回选项",
	"approval.prefixPrompt": "编辑要记住的规则（留空 = 仅允许这一次）：",
	"approval.prefixHint": "Enter 保存规则并允许 • Esc 返回选项",
	"approval.grantedSession": "本会话内不再询问 {tool}（仅此终端）。重启或恢复会话后将再次询问。",
	"approval.grantedProject": "本项目内此后不再询问 {tool}。规则存于 {path}，编辑该文件可撤销。",
	"approval.grantedProjectAccess": "本项目内此后不再询问 {tool} 的 {access} 权限请求。规则存于 {path}，编辑该文件可撤销。",
	"approval.grantedPrefix": "本项目内此后允许匹配 {rule} 的命令。规则存于 {path}，编辑该文件可撤销。",
	"approval.diffClipped": "… （另有 {count} 行改动 • 其余未显示）",
	"approval.diffMoreFiles": "… 本次调用还会改动另外 {count} 个文件",
	"approval.diffNoChanges": "无改动",
	"approval.diffApproximate": "已省略精确行级 diff：改动超过 {limit} 行",
	...
	"notice.promptTruncated": "这条输入有 {original} 字符，上限是 {limit}，发送前已丢弃中间 {removed} 字符。长文件请用 @路径 附带，不要直接粘贴；也可以调大 maxPromptChars。",
	...
	"login.select.title": "登录一个 provider",
	"login.select.lead": "已配置的 provider 只是换一把新 key；其余的会在登录时写入 settings。",
	"login.group.configured": "已配置",
	"login.group.available": "可配置",
	"login.roster.noKey": "没有保存 key",
	"login.roster.keyIn": "key 在 {env}",
	"login.key.title": "登录 {provider}",
	"login.key.catalogEndpoint": "这个 provider 的 endpoint 来自内置目录，所以这里无法验证 key。",
	"login.key.endpoint": "Endpoint：{url}",
	"login.key.storage": "key 只保存在 credential store，不会写进 settings。",
	"login.key.prompt": "粘贴 API key，输入内容不会回显。",
	"login.key.rejected": "endpoint 拒绝了这把 key：{message}。什么都没有保存。",
	"login.status.checking": "正在用这把 key 请求 endpoint…",
	"login.status.saving": "正在保存…",
	"login.unverified.title": "key 未经验证",
	"login.unverified.reassure": "保存本身没有风险，只是还没有证明它能用。",
	"login.unverified.save": "仍然保存这把 key",
	"login.unverified.saveDetail": "第一次使用时再验证",
	"login.unverified.cancel": "取消",
	"login.unverified.cancelDetail": "什么都不写",
	"login.saved.added": "已添加 provider",
	"login.saved.signedIn": "已登录",
	"login.saved.summary": "{what} {provider}，key 为 {key}。",
	"login.saved.credential": "key 以 {ref} 保存在 credential store，settings 里只记下这个变量名。",
	"login.saved.verified": "endpoint 已接受这把 key。",
	"login.saved.unverified": "这把 key 没有向 endpoint 验证过。",
	"login.effect.live": "现在就已生效 —— /model 里能看到它的 model。",
	"login.effect.restart": "重启 dsh 后生效。",
	"login.effect.unknown": "这个部署没有说明它是否需要重启才生效；如果 /model 里看不到，就重启 dsh。",
	"login.noProviders": "没有已配置的 provider，也没有任何已注册的 adapter 提供。用 /provider add 手动描述一个。",
	"login.noStore": "这个部署没有可写的 settings 或 credential store，key 无法在这里保存。",
	"login.unknownProvider": "未知的 provider：{name}。运行 /login 可以看到全部。",
	"login.error.save": "保存失败：{error}",
	"login.error.start": "没能开始登录：{error}",
	"login.refuse.keyEmpty": "请输入 key，或按 Esc 取消。",
	...
	"provider.add.lead": "会写入 {ns} 这个 settings 段。",
	"provider.add.prompt": "provider 名，也就是 /model 里写的那个（例如 acme）。",
	"provider.add.taken": "{value} 已经配置过了；用 /login 给它换一把新 key。",
	"provider.endpoint.title": "Endpoint",
	"provider.endpoint.lead": "adapter 会在这个 base URL 后面接自己的路径。",
	"provider.endpoint.prompt": "Base URL，例如 https://api.example.com/v1",
	"provider.endpoint.refuseEmpty": "请输入 URL，或按 Esc 取消。",
	"provider.endpoint.refuseShape": "请输入 http:// 或 https:// 开头的 URL。",
	"provider.protocol.title": "通信协议",
	"provider.protocol.lead": "只有 OpenAI 形状的 endpoint 能被问出 model 列表，其余的保存时不做验证。",
	"provider.protocol.noListing": "不支持列出 model",
	"provider.probe.unsupported": "{api} 这类 endpoint 不列出自己的 model，所以这里无法验证 key。",
	"provider.probe.silentFailure": "endpoint 失败了，但没有说原因",
	"provider.displayName.title": "显示名",
	"provider.displayName.lead": "在各个选择器里显示。留空就用 provider 名。",
	"provider.displayName.prompt": "显示名（可选）。",
	"provider.credential.title": "credential 变量名",
	"provider.credential.lead": "settings 里只记这个变量名，密钥本身存在 credential store。",
	"provider.credential.prompt": "环境变量名。",
	"provider.key.title": "{route} 的 key",
	"provider.key.storedAs": "将以 {ref} 保存在 credential store。",
	"provider.model.title": "Model",
	...
	"provider.refuse.credentialEmpty": "请输入变量名，或按 Esc 取消。",
	"provider.refuse.credentialShape": "环境变量名以字母或下划线开头，只能包含字母、数字和下划线。",
	"dialog.provider.empty": "没有可选项。",
	"dialog.provider.hintBusy": "Esc 取消",
	"dialog.provider.hintText": "Enter 继续 · Ctrl+U 清空 · Esc 取消",
	"dialog.provider.hintChecklist": "↑↓ 移动 · Space 勾选 · Enter 继续 · Esc 取消",
	"dialog.provider.hintSelect": "↑↓ 移动 · Enter 选择 · Esc 取消"
};
//#endregion
...
*/
function brandSchemeColors(scheme) {
	return scheme === "light" ? {
		userMessageBg: rgb(240, 240, 240),
		planMode: rgb(20, 70, 145),
...
fgAnsi(BRAND_COLORS.diffSafeMuted);
...
const DEFAULT_MAX_DIFF_EDIT_LENGTH = 1e3;
/**
...
function permissionAccent(palette, text) {
	return palette.bold("x") === "x" ? text : fg(BRAND_COLORS.permission, text);
}
/**
...
*/
function diffContentLines$1(text) {
	if (text === "") return [];
...
*/
function parseFileDiff(diff, maxEditLength) {
	...
	const oldLines = diffContentLines$1(oldText);
	const newLines = diffContentLines$1(newText);
	return {
		diff: {
...
var ApprovalDialog = class {
	...
	*/
	diffCache;
	focused = false;
	constructor(prompt, palette, done, limits = {}) {
		this.prompt = prompt;
	...
	render(width) {
		...
		if (commandCwd !== void 0 && commandCwd !== "") body.push(...wrapTextWithAnsi(this.palette.dim(t("approval.commandCwd", { path: displayInlineText(commandCwd) })), inner));
	...
	*/
	options() {
		if (this.prompt.command !== void 0) {
			const suggestion = this.suggestion();
			if (suggestion === void 0) return ANSWER_OPTIONS;
			return [...ANSWER_OPTIONS, {
				action: "allow-prefix",
				label: () => this.prefixLabel(suggestion)
			}];
		}
		const diffs = this.prompt.diffs;
		if (diffs === void 0 || diffs.length === 0) return ANSWER_OPTIONS;
		return [...ANSWER_OPTIONS, {
			action: "allow-project",
			label: (toolName) => this.projectLabel(toolName)
		}];
	}
	/**
	...
	*/
	projectLabel(toolName) {
		const access = this.prompt.access;
		if (access === void 0) return t("approval.allowProject", { tool: toolName });
		return t("approval.allowProjectAccess", {
			tool: toolName,
			access: displayInlineText(access)
		});
	}
	/**
	...
	* @returns the row's text.
	*/
	prefixLabel(suggestion) {
		const prefix = displayInlineText(suggestion);
		const access = this.prompt.access;
		if (access === void 0) return t("approval.allowPrefix", { prefix });
		return t("approval.allowPrefixAccess", {
			prefix,
			access: displayInlineText(access)
		});
	}
	...
	*/
	suggestion() {
		...
		return exact === "" || isCompoundCommand(exact) ? void 0 : exact;
	}
	/**
	...
	*/
	renderDiffPreview(inner, headRows) {
		const diffs = this.prompt.diffs;
		if (diffs === void 0 || diffs.length === 0) return [];
		const budget = this.previewBudget(headRows);
		const cached = this.diffCache;
		if (cached !== void 0 && cached.width === inner && cached.budget === budget) return cached.rows;
	...
	*/
	buildDiffPreview(diffs, inner, budget) {
		...
		const tailRows = () => FILE_SUMMARY_ROWS * (named - 1) + (named < diffs.length ? 1 : 0);
	...
	*/
	renderFileDiff(diff, inner, budget, maxEditLength) {
		const rows = [this.pathRow(diff, inner)];
		if (budget < FILE_SUMMARY_ROWS) return rows;
		const parsed = parseFileDiff(diff, maxEditLength);
		const note = parsed.approximate ? [this.palette.dim(t("approval.diffApproximate", { limit: maxEditLength }))] : [];
		const forHunks = budget - 1 - note.length;
		const hunks = inner < DIFF_PREVIEW_MIN_WIDTH || forHunks < MIN_UNIFIED_ROWS ? [] : this.renderHunks(parsed.diff, inner, forHunks, diffLanguage(diff.path));
		if (hunks.length === 0) {
	...
	*/
	renderHunks(parsed, inner, budget, language) {
		...
		while (lines >= 1) {
			...
			const rows = (clipped ? rendered.slice(0, -1) : rendered).map((row) => ({ fitted: row }));
	...
	*/
	previewBudget(headRows) {
		const height = this.limits.maxHeight;
		if (height === void 0) return DIFF_PREVIEW_MAX_ROWS;
...
var HintEditor = class extends Editor {
	...
	*/
	seedHistory(newestFirst) {
		for (let index = newestFirst.length - 1; index >= 0; index--) {
			const text = newestFirst[index];
			if (text === void 0) continue;
			this.addToHistory(text, { persist: false });
		}
	}
	/**
	...
	*/
	addToHistory(text, options) {
		...
		if (options?.persist !== false) this.onHistoryAdd?.(trimmed);
	}
	render(width) {
...
//#endregion
//#region src/components/tool-presenters.ts
/** The questions array from an ask_user_question call, or undefined off-shape. */
function askQuestions(args) {
	if (typeof args !== "object" || args === null) return void 0;
	const { questions } = args;
	if (!Array.isArray(questions)) return void 0;
	return questions.filter((q) => typeof q === "object" && q !== null);
}
/**
...
*/
function askCallView(args) {
	const questions = askQuestions(args);
	const first = questions?.[0];
	const label = first?.header ?? first?.question;
	return {
		card: "generic",
		title: label === void 0 ? "question" : questions !== void 0 && questions.length > 1 ? `${label} (+${questions.length - 1} more)` : label
	};
}
/**
...
*/
function askResultView(args, result) {
	const text = contentText(result.content);
	if (result.isError) {
		if (!text.includes("interrupted before the user answered")) return void 0;
		return {
			card: "generic",
			content: [{
				type: "text",
	...
	try {
		...
		const items = typeof parsed === "object" && parsed !== null ? parsed.answers : void 0;
		if (!Array.isArray(items)) return void 0;
		answers = items.filter((a) => typeof a === "object" && a !== null);
	} catch {
		return;
	}
	const rows = answers.map((answer) => {
		...
		const picked = [...answer.selected ?? [], ...answer.custom === void 0 || answer.custom === "" ? [] : [answer.custom]];
	...
	return {
		...
		content: [{
			type: "text",
			text: ["User answered:", ...rows].join("\n")
		}]
	};
}
/** Fallback presenters, keyed by tool name. */
const FALLBACK_PRESENTERS = { ask_user_question: {
	presentCall: askCallView,
	presentResult: askResultView
} };
/**
...
*/
function withTuiPresenters(name, definition) {
	const fallback = FALLBACK_PRESENTERS[name];
	if (fallback === void 0 || definition === void 0) return definition;
	if (definition.presentCall !== void 0 && definition.presentResult !== void 0) return definition;
	return {
		...definition,
		...definition.presentCall === void 0 ? { presentCall: fallback.presentCall } : {},
		...definition.presentResult === void 0 ? { presentResult: fallback.presentResult } : {}
	};
}
//#endregion
...
*/
function workflowRunStatus(node) {
	switch (node.stopReason) {
		case "completed": return "completed";
		case "cancelled": return "cancelled";
		case "error": return "failed";
		default: return node.endedAt === void 0 ? "running" : "interrupted";
	}
}
/**
...
* @returns the derived status.
*/
function workflowMemberStatus(node, member) {
	if (member.outcome !== void 0) return member.outcome;
	return workflowRunStatus(node) === "running" ? "running" : "interrupted";
}
/**
...
*/
function groupWorkflowPhases(node) {
	const named = /* @__PURE__ */ new Map();
	const missing = [];
	const order = [];
	for (const member of node.members) {
		if (member.phase === void 0) {
			if (missing.length === 0) order.push(void 0);
			missing.push(member);
			continue;
		}
		const bucket = named.get(member.phase);
		if (bucket === void 0) {
			named.set(member.phase, [member]);
	...
	return order.map((phase) => ({
		phase,
		members: phase === void 0 ? missing : named.get(phase) ?? []
	}));
}
/**
...
*/
function workflowStatusCounts(node, members) {
	const counts = {
		running: 0,
		completed: 0,
		failed: 0,
	...
	for (const member of members) counts[workflowMemberStatus(node, member)] += 1;
	return counts;
}
/**
...
*/
function workflowNeedsAttention(node, members) {
	return members.some((member) => workflowMemberStatus(node, member) !== "completed");
}
//#endregion
...
*/
function packSkillNames(names, width, maxRows = SKILLS_MAX_ROWS) {
...
var HeaderComponent = class {
	...
	*/
	...
	invalidate() {}
	render(width) {
		const rows = width < MIN_BOXED_WIDTH ? this.renderPlain(width) : (width >= FULL_MIN_WIDTH ? this.renderFull(width) : void 0) ?? this.renderBoxed(width);
		const reveal = this.revealWidth;
		if (reveal === void 0) return rows;
	...
	wordmark() {
		...
		return `${name} ${this.palette.bold("HARNESS")}` + (version === void 0 ? "" : ` ${this.palette.dim(`v${displayText(version)}`)}`);
	}
	/** The `resumed <id> · <title>` line, or `undefined` on a fresh session. */
	resumedLine() {
		if (this.info.resumed === void 0) return void 0;
		const title = this.info.title();
		return t("banner.resumed", { id: displayText(this.info.resumed) }) + (title === void 0 ? "" : ` · ${displayText(title)}`);
	}
	/**
	...
	*/
	renderFull(width) {
		if (this.info.skills === void 0) return void 0;
		const dim = (text) => this.palette.dim(text);
		const inner = width - 3;
		const model = this.info.model();
		const resumed = this.resumedLine();
		const identity = [
			...model === void 0 ? [] : [displayText(model)],
			displayText(this.info.cwd),
			...resumed === void 0 ? [] : [resumed]
		];
		const welcome = this.info.welcome === void 0 ? void 0 : displayText(this.info.welcome);
		const leftWidth = Math.min(Math.max(20, ...[...identity, ...welcome === void 0 ? [] : [welcome]].map((line) => visibleWidth(line) + 4)), FULL_LEFT_MAX, inner - FULL_RIGHT_MIN - 1);
		const rightWidth = inner - leftWidth - 1;
		if (leftWidth < 16) return void 0;
		const centered = (text) => {
		...
		const identityLead = " ".repeat(Math.max(1, Math.floor((leftWidth - Math.min(Math.max(...identity.map((line) => visibleWidth(line))), leftWidth - 2)) / 2)));
		const left = [
			"",
			...welcome === void 0 ? [] : [centered(this.palette.bold(welcome)), ""],
		...
		const section = (label, names) => names.length === 0 ? [] : [` ${this.palette.bold(this.palette.accent(label))}`, ...packSkillNames(names, rightWidth - 2).map((row) => ` ${dim(row)}`)];
		...
		const rows = [rule < 1 ? ` ${dim(`╭${"─".repeat(inner)}╮`)}` : ` ${dim("╭─")} ${wordmark} ${dim(`${"─".repeat(rule)}╮`)}`];
		const height = Math.max(left.length, right.length);
		const drop = Math.floor((height - left.length) / 2);
		for (let index = 0; index < height; index += 1) rows.push(` ${dim("│")}${cell(left[index - drop] ?? "", leftWidth)}${dim("│")}${cell(right[index] ?? "", rightWidth)}${dim("│")}`);
	...
	*/
	renderBoxed(width) {
		...
		const lines = [
			this.wordmark(),
			...model === void 0 ? [] : [this.palette.dim(displayText(model))],
			this.palette.dim(displayText(this.info.cwd)),
			...resumed === void 0 ? [] : [this.palette.dim(resumed)]
		];
		...
		for (let index = 0; index < WHALE_ART.length || index < lines.length; index += 1) {
			...
			rows.push(` ${this.palette.dim("│")} ${this.gradient() ? brandText(art) : this.palette.brand(art)}  ${text}${pad} ${this.palette.dim("│")}`);
		}
	...
	*/
	renderPlain(width) {
		...
		return [...[
			...
			...resumed === void 0 ? [] : detail(resumed)
		].map((line) => line === "" ? "" : ` ${line}`), ...this.trailer(width)];
	}
	/**
	...
	*/
	trailer(width) {
		...
		return [
			...welcome === void 0 ? [] : wrapTextWithAnsi(this.palette.dim(displayText(welcome)), usable).map((line) => truncateToWidth(line, usable, "")),
	...
	*/
	sectionRows(usable, label, names) {
		if (names.length === 0) return [];
		return [
			"",
...
function assistantMessageChildren(content, showThinking, palette, mdTheme, markdown) {
	...
	if (text !== "") {
		...
		children.push(new PrefixedComponent(document, `${GUTTER}${accent(palette, BRAND_COLORS.brand, "●")} `, GUTTER_INDENT));
	}
	return children;
}
/**
...
* @returns The badge row, ready to render above the prompt.
*/
function planModeRow(palette, scheme = "dark", hint, pending = false) {
	return modeRow(palette, brandSchemeColors(scheme).planMode, `${PLAN_MODE_ICON} ${t(pending ? "transcript.planModeBadgePending" : "transcript.planModeBadge")}`, hint);
}
/**
...
function autoAcceptRow(palette, scheme = "dark", hint) {
	return modeRow(palette, brandSchemeColors(scheme).autoAccept, `${AUTO_ACCEPT_ICON} ${t("transcript.autoAcceptBadge")}`, hint);
}
/**
...
var ToolCardComponent = class extends CachedCardComponent {
	...
	*/
	headerRow(width) {
		...
		const status = this.result === void 0 ? BRAND_COLORS.brand : isError ? BRAND_COLORS.error : BRAND_COLORS.success;
...
var CollapsedGroupComponent = class extends CachedCardComponent {
	...
	*/
	...
	renderLines(width) {
		...
		const bullet = group.active ? this.palette.bold(accent(this.palette, group.failed ? BRAND_COLORS.error : BRAND_COLORS.brand, TOOL_BULLET)) : group.failed ? accent(this.palette, BRAND_COLORS.error, TOOL_BULLET) : " ";
...
*/
var WorkflowRunComponent = class extends CachedCardComponent {
	node;
	visibility;
	palette;
	expandKey;
	...
	*/
	constructor(node, visibility, palette, expandKey, now) {
		super();
		this.node = node;
		this.visibility = visibility;
		this.palette = palette;
	...
	*/
	...
	renderLines(width) {
		...
		const openPhase = (group) => expandAll || workflowNeedsAttention(node, group.members);
		const hidden = phases.length > 0 && (!showPhases || phases.some((group) => !openPhase(group) && group.members.length > 0));
		const rows = ["", ...this.headRows(node, status, this.visibility === "collapsed" && hidden, width)];
		if (showPhases) for (const group of phases) {
	...
	*/
	headRows(node, status, hint, width) {
		const { glyph, color } = WORKFLOW_GLYPHS[status];
		const bullet = status === "running" ? this.palette.bold(accent(this.palette, color, glyph)) : accent(this.palette, color, glyph);
		const summary = [
		...
		const head = hint ? `${text} ${this.palette.dim(t("collapse.expandHint", { key: this.expandKey().toLowerCase() }))}` : text;
		const rows = [];
		let first = true;
		for (const row of wrapTextWithAnsi(head, Math.max(20, width - GUTTER_WIDTH - 2))) {
			rows.push(first ? `${GUTTER}${bullet} ${row}` : `${GUTTER_INDENT}${row}`);
	...
	*/
	phaseRow(node, group, width) {
		const name = group.phase === void 0 ? t("workflow.phase.none") : group.phase === "" ? t("workflow.phase.unnamed") : displayInlineText(group.phase);
		const counts = workflowStatusCounts(node, group.members);
		const attention = WORKFLOW_ATTENTION_STATUSES.filter((status) => counts[status] > 0).map((status) => t(`workflow.count.${status}`, { count: counts[status] }));
		const parts = attention.length === 0 ? [t("workflow.count.completed", { count: counts.completed })] : counts.completed > 0 ? [t("workflow.count.completed", { count: counts.completed }), ...attention] : attention;
		const summary = [
			name,
			plural(group.members.length, "workflow.members"),
			...parts
		].join(t("workflow.separator"));
	...
	*/
	memberRow(node, member, width) {
		const status = workflowMemberStatus(node, member);
		const { glyph, color } = WORKFLOW_GLYPHS[status];
		const bullet = accent(this.palette, color, glyph);
		const text = [member.label === "" ? t("workflow.member.unnamed") : displayInlineText(member.label), this.statusText(status, member.startedAt, member.endedAt ?? node.endedAt)].join(t("workflow.separator"));
	...
	*/
	statusText(status, startedAt, endedAt) {
...
*/
var CompactionSummaryComponent = class extends CachedCardComponent {
	summary;
	palette;
	constructor(summary, palette) {
		super();
...
var TodoComponent = class {
	...
	*/
	...
	renderItem(todo, width) {
		const icon = todo.status === "completed" ? accent(this.palette, BRAND_COLORS.success, "✔") : todo.status === "in_progress" ? accent(this.palette, BRAND_COLORS.brand, "◼") : this.palette.dim("◻");
...
*/
function userBadge(node) {
	if (node.source === "queued") return node.optimistic === true ? t("transcript.queuedBadge") : void 0;
	if (node.source !== "steering") return void 0;
...
var TranscriptReconciler = class {
	...
	*/
	reconcile(nodes) {
		...
		for (let index = this.hiddenBefore; index < nodes.length; index += 1) {
			...
			flushFooter();
			if (node.kind === "user-message" ? node.source === "user" : node.kind === "assistant" && node.turn !== turn) flushTurn();
			...
			switch (node.kind) {
				...
				case "compaction": {
					...
					seen.add(markerKey);
					const hint = hints ? ` ${t("collapse.expandHint", { key: this.deps.expandKey().toLowerCase() })}` : "";
					children.push(this.plainView(markerKey, node.version, () => block(new Text(this.deps.palette.dim(t("transcript.compactionMarker") + hint), 0, 0))));
					if (opened) {
						const summaryKey = `${node.key}:summary`;
						seen.add(summaryKey);
						children.push(this.plainView(summaryKey, node.version, () => block(new CompactionSummaryComponent(node.summary, this.deps.palette))));
					}
					break;
				}
				case "context":
	...
	*/
	userView(node) {
		...
		if (badge === void 0) return block(body);
		const container = new Container();
		container.addChild(new Spacer(1));
	...
	*/
	workflowView(node) {
		const existing = this.views.get(node.key);
		if (existing !== void 0 && existing.kind === "workflow") {
			if (existing.version !== node.version) {
				existing.version = node.version;
		...
		const component = new WorkflowRunComponent(node, this.visibility, this.deps.palette, this.deps.expandKey, this.deps.now);
		this.views.set(node.key, {
			kind: "workflow",
			version: node.version,
	...
	*/
	toolView(node) {
		...
		const component = new ToolCardComponent(node.name, node.args, withTuiPresenters(node.name, this.deps.toolDefinition(node.name)), this.deps.maxToolOutputLines, this.deps.maxDiffEditLength, this.deps.palette, this.deps.mdTheme, this.deps.expandKey);
...
function findWorkflowRun(nodes, runId) {
...
*/
function closeOpenWorkflowRuns(nodes, time) {
	let changed = false;
	for (const node of nodes) {
		if (node.kind !== "workflow-run") continue;
		if (node.stopReason !== void 0 || node.endedAt !== void 0) continue;
		node.endedAt = time;
		changed = touch(node);
	}
	return changed;
...
function foldEvent(nodes, event) {
	...
	switch (event.type) {
		...
		case "turn/start": {
			...
			const existing = findTodo(nodes);
			if (existing === void 0 || existing.todos.length === 0) return changed;
			existing.todos = [];
			return touch(existing);
		}
		...
		case "tool-workflow/agent-end": {
			...
			if (node === void 0 || entry === void 0) return false;
		...
		case "compaction/end": {
			...
			if (event.data.sourceCommandId !== void 0) return false;
...
*/
function pressureLevel(percentRemaining) {
...
*/
function rearmLevel(percentRemaining) {
	if (percentRemaining <= 13) return "critical";
	if (percentRemaining <= 28) return "low";
	return "normal";
}
/**
...
*/
function contextPressure(used, window) {
	if (window === void 0 || !Number.isFinite(window) || window <= 0) return void 0;
	...
	return {
		...
		percentUsed,
		percentRemaining,
		level: pressureLevel(percentRemaining)
	};
}
/**
...
*/
function createContextAnnouncementTracker() {
...
*/
function nextContextAnnouncement(tracker, pressure) {
	...
	return level === "normal" ? void 0 : level;
}
//#endregion
//#region src/chat/compact.ts
/** Every code the seam declares, for the duck-typed recognition below. */
const MANUAL_COMPACTION_CODES = /* @__PURE__ */ new Set([
	"busy",
	"cancelled",
	"changed",
	"summary",
...
*/
function manualCompactionCode(error) {
	if (!(error instanceof Error)) return void 0;
	if (error.name !== "ManualCompactionError") return void 0;
	const code = error.code;
	return typeof code === "string" && MANUAL_COMPACTION_CODES.has(code) ? code : void 0;
}
/**
...
*/
function compactFailureText(code) {
	switch (code) {
		case "busy": return t("compact.error.busy");
		case "cancelled": return t("compact.cancelled");
		case "changed": return t("compact.error.changed");
		case "summary": return t("compact.error.summary");
		case "commit": return t("compact.error.commit");
		/* v8 ignore next 2 -- the code set is closed; the default is a future-code backstop. */
		default: return t("compact.error.persistence");
	}
}
/**
...
*/
async function runCompactCommand(deps, rawInput, signal, commandId) {
	if (rawInput.trim() !== "") return {
		kind: "error",
		text: t("compact.usage")
	};
	const engine = deps.engine();
	if (engine === void 0) return {
		kind: "error",
		text: t("compact.unavailable")
	};
	...
	try {
		...
		if (signal.aborted) return {
			kind: "error",
			text: t("compact.cancelled")
		};
		const code = manualCompactionCode(error);
		if (code !== void 0) return {
			kind: "error",
			text: compactFailureText(code)
		};
		return {
			kind: "error",
			text: t("compact.failed", { error: errorChain(error) })
		};
	}
}
//#endregion
...
//#endregion
//#region src/chat/prompt-truncation.ts
/**
...
function truncatePrompt(text, limit) {
	...
	if (limit <= 0 || original <= limit) return {
		text,
		original,
		removed: 0
	};
...
function resolveTuiConfig(config) {
	return {
		...
		maxPromptChars: config?.maxPromptChars ?? 1e4,
...
function forkSeedLength(events, seq) {
	...
	for (const [index, event] of events.entries()) {
		...
		if (event.type === "turn/start") inTurn = true;
		else if (event.type === "turn/end") {
			inTurn = false;
			cut = index + 1;
		} else if (!inTurn) cut = index + 1;
	}
	return cut;
}
//#endregion
...
const APP_KEYBINDINGS = {
	...
	"app.submit.opposite": {
		...
		description: "Send with the opposite busy-Enter behavior"
	},
	"app.cancel": {
		defaultKeys: "escape",
...
var SkillsPanel = class {
	...
	*/
	...
	viewport() {
		return Math.max(1, this.rows() - LIST_CHROME_ROWS$1);
	}
	detailViewport() {
...
*/
function subagentCounts(entries) {
	let running = 0;
	for (const entry of entries) if (entry.kind === "child" && entry.activity === "running") running += 1;
	return {
		running,
...
function subagentName(entry) {
	return entry.kind === "child" && entry.label !== void 0 && entry.label !== "" ? entry.label : entry.id;
}
//#endregion
//#region src/components/subagents-panel.ts
/**
...
*/
/** The panel's heading, so the command and its view name the same thing. */
const SUBAGENTS_PANEL_TITLE = "/subagents";
t("subagents.unavailable", void 0, "en");
t("subagents.loading", void 0, "en");
t("subagents.empty", void 0, "en");
...
*/
function statusMark$1(entry, palette) {
...
function secondaryText$1(entry) {
	if (entry.kind === "diagnostic") return t(`subagents.diagnostic.${entry.reason}`);
	return `${entry.mode === "one-shot" ? t("subagents.mode.oneShot") : t("subagents.mode.continuable")} · ${t(`subagents.activity.${entry.activity}`)}`;
}
/**
...
*/
function renderSubagentRows(entries, palette, width) {
	const names = entries.map((entry) => ({
		indent: " ".repeat(Math.max(0, entry.depth - 1) * INDENT_WIDTH),
		name: displayInlineText(subagentName(entry))
	}));
	const nameColumn = Math.min(Math.max(1, ...names.map(({ indent, name }) => indent.length + visibleWidth(name))), Math.max(8, Math.floor(width / 2)));
	return entries.map((entry, index) => {
		const { indent, name } = names[index] ?? {
			indent: "",
			name: ""
		};
		...
		const secondary = displayInlineText(secondaryText$1(entry));
		const id = displayInlineText(entry.id);
		return truncateToWidth(`${indent}${statusMark$1(entry, palette)} ${palette.text(painted)}${pad}  ${palette.dim(secondary)}  ${palette.dim(id)}`, width, "");
	});
}
/**
...
var SubagentsPanel = class {
	...
	*/
	...
	renderMessage(title, message, width) {
		const viewport = Math.max(1, this.rows() - MESSAGE_CHROME_ROWS$1);
		return [
			"",
			title,
	...
	render(width) {
		...
		if (entries === void 0) return this.error === void 0 ? this.renderMessage(title, this.palette.dim(t("subagents.loading")), contentWidth) : this.renderMessage(title, this.palette.error(displayInlineText(this.error)), contentWidth);
		if (entries.length === 0 && this.error === void 0) return this.renderMessage(title, this.palette.dim(t("subagents.empty")), contentWidth);
		const counts = subagentCounts(entries);
		const count = plural(counts.total, "subagents.count", { ...counts });
		const rows = [...this.error === void 0 ? [] : wrapTextWithAnsi(this.palette.error(displayInlineText(this.error)), contentWidth), ...renderSubagentRows(entries, this.palette, contentWidth)];
		const viewport = this.viewport();
		this.offset = Math.max(0, Math.min(this.offset, Math.max(0, rows.length - viewport)));
		const shown = rows.slice(this.offset, this.offset + viewport);
		const position = rows.length > viewport ? `  ·  ${t("panel.position", {
...
function jobsRegistry(ctx) {
	...
	return typeof registry.list === "function" && typeof registry.onJobsChanged === "function" ? registry : void 0;
}
/**
...
function jobCounts(rows) {
	let live = 0;
	for (const row of rows) if (isLiveJob(row)) live += 1;
	return {
		live,
		total: rows.length
	};
...
*/
function sortJobRows(rows) {
	return [...rows].sort((left, right) => {
		const liveLeft = isLiveJob(left);
		if (liveLeft !== isLiveJob(right)) return liveLeft ? -1 : 1;
		if (liveLeft) return left.startedAt - right.startedAt || left.id.localeCompare(right.id);
		const endLeft = left.finishedAt ?? left.startedAt;
		return (right.finishedAt ?? right.startedAt) - endLeft || left.id.localeCompare(right.id);
	});
}
/**
...
*/
function jobElapsed(row, now) {
	return Math.max(0, (row.finishedAt ?? now) - row.startedAt);
}
//#endregion
//#region src/components/jobs-panel.ts
/**
...
*/
/** The panel's heading, so the command and its view name the same thing. */
const JOBS_PANEL_TITLE = "/jobs";
t("jobs.unavailable", void 0, "en");
t("jobs.empty", void 0, "en");
...
function statusMark(row, palette) {
...
function secondaryText(row) {
	...
	return row.detail === void 0 || row.detail === "" ? status : `${status} · ${displayInlineText(row.detail)}`;
}
/**
...
*/
function renderJobRows(rows, palette, width, now) {
	...
	const labelColumn = Math.min(Math.max(1, ...labels.map(visibleWidth)), Math.max(8, Math.floor(width / 2)));
	return rows.map((row, index) => {
		...
		return truncateToWidth(`${statusMark(row, palette)} ${palette.dim(kind)}${kindPad}  ${palette.text(label)}${labelPad}  ${palette.dim(secondaryText(row))}  ${palette.dim(formatTurnDuration(jobElapsed(row, now)))}`, width, "");
	});
}
/**
...
var JobsPanel = class {
	...
	* @param onClose - called on Esc or Ctrl+C; the caller closes the overlay.
	*/
	constructor(jobs, rows, palette, now, onClose) {
		this.jobs = jobs;
		this.rows = rows;
		this.palette = palette;
		this.now = now;
	...
	render(width) {
		...
		if (this.jobs.length === 0) {
			...
			return [
				"",
				title,
				...wrapTextWithAnsi(this.palette.dim(t("jobs.empty")), contentWidth).slice(0, viewport).map((line) => ` ${line}`),
				` ${this.palette.dim(t("panel.escClose"))}`
			];
		}
...
*/
const BUSY_ENTER_BEHAVIORS = ["steer", "queue"];
/**
...
const TUI_PREFERENCES_SCHEMA = z.object({
	...
	queueUpHintSeen: z.number().step(1).min(0).max(3).default(0),
...
function normalize(value, fallback) {
	...
	return {
		...
		queueUpHintSeen: typeof hintSeen === "number" && Number.isFinite(hintSeen) && hintSeen >= 0 ? Math.min(3, Math.trunc(hintSeen)) : fallback.queueUpHintSeen,
...
t("dialog.question.customAnswerLabel", void 0, "en");
/**
...
var PresetDialog = class {
	...
	*/
	describeChoice(choice) {
		...
		return [
			...choice.broken === void 0 ? [] : [t("preset.broken.badge", { reason: displayText(choice.broken) })],
			badges,
...
var QuestionDialog = class {
	...
	handleInput(data) {
		...
		const options = this.options;
		if (matchesKey(data, Key.space) && this.question.multiSelect) this.toggleOption(this.selectedIndex);
		else if (matchesKey(data, Key.enter)) this.submitOption(this.selectedIndex);
	...
	submitOption(index) {
		const selected = this.question.multiSelect ? this.selectedOptionLabels() : [this.options[index]?.label].filter((label) => label !== void 0);
		const custom = this.question.multiSelect ? this.input.getValue().trim() : "";
		if (selected.length === 0 && custom === "") {
			this.error = t("dialog.question.selectOne", { label: t("dialog.question.customAnswerLabel") });
			return;
		}
		this.done({
			selected,
	...
	render(width) {
		...
		const progress = this.total > 1 ? this.palette.dim(`  ${t("dialog.question.progress", {
			position: this.position,
			total: this.total,
			unanswered: this.unanswered
		})}`) : "";
		const questionLines = wrapTextWithAnsi(this.palette.bold(this.palette.text(displayText(this.question.question))), innerWidth);
		const contentLines = [...questionLines];
		const headerLines = [...wrapTextWithAnsi(`${chip}${progress}`, innerWidth), ...questionLines];
		...
		const controls = [
			...
			...this.inputIndex > 0 ? [t("dialog.question.navigate")] : [],
			...this.question.multiSelect ? [t("dialog.question.spaceToggle")] : [],
			t("dialog.question.escCancel")
		];
		...
		const positionLines = this.options.length > this.maxVisible ? [this.palette.dim(`${this.selectedIndex + 1}/${this.inputIndex + 1}`)] : [];
		const paddingRows = 2;
		const maxHeight = this.maxHeight();
		const availableForOptions = Math.max(4, maxHeight - paddingRows - headerLines.length - positionLines.length - footerLines.length);
		const body = [...headerLines];
		const optionLines = [];
		const optionBlocks = [...this.options.map((option, index) => this.renderOptionBlock(option, index, innerWidth)), this.renderInputBlock(innerWidth)];
	...
	*/
	renderInputBlock(innerWidth) {
		...
		const label = t("dialog.question.customAnswerLabel");
		const value = this.input.getValue();
		const body = value === "" ? this.palette.dim(this.question.multiSelect ? label.replace(/[.。]$/u, "") : label) : truncateToWidth(displayText(value), bodyWidth, "…");
	...
	compactOptionControls(innerWidth, showPager = false) {
		...
		const fallback = showPager ? `P↑↓ ${optionNavigation}${this.question.multiSelect ? "S " : ""}↵Esc` : this.question.multiSelect ? `${optionNavigation}Sp ↵Esc` : `${optionNavigation}↵ Esc`;
...
*/
function renderSkillEcho(name, instructions) {
...
*/
function collectAnswerTexts(nodes) {
	const answers = [];
	for (let index = nodes.length - 1; index >= 0; index -= 1) {
		const node = nodes[index];
...
*/
function parseCopyArgument(rawInput) {
...
//#endregion
//#region src/chat/prompt-history.ts
/**
...
const MAX_PERSISTED_LENGTH = 1e5;
...
*/
const PROMPT_HISTORY_FLUSH_TIMEOUT_MS = 1e3;
...
*/
function readTailLines(path, windowBytes) {
	let fd;
	try {
		fd = openSync(path, "r");
	} catch (_noHistoryYet) {
...
function parseRecord(line) {
	...
	const record = value;
	if (typeof record.display !== "string") return void 0;
	if (typeof record.timestamp !== "number") return void 0;
	if (typeof record.cwd !== "string") return void 0;
	if (typeof record.sessionId !== "string") return void 0;
	const bodyHash = record.bodyHash;
	if (bodyHash !== void 0 && !(typeof bodyHash === "string" && /^[0-9a-f]{16}$/u.test(bodyHash))) return;
	return {
		display: record.display,
		timestamp: record.timestamp,
		cwd: record.cwd,
		sessionId: record.sessionId,
		...bodyHash === void 0 ? {} : { bodyHash },
...
*/
async function compactPromptHistory(path, options = {}) {
	...
	return await withFileLock(path, async () => {
		...
		const lines = raw.split("\n").filter((line) => line.trim() !== "" && parseRecord(line) !== void 0);
		if (lines.length <= keep) return void 0;
		...
		for (const line of [...kept, ...tail.split("\n")]) {
			...
			if (hash !== void 0) referenced.add(hash);
		}
...
*/
async function readBytes(path, start, end) {
...
*/
async function sweepBodies(directory, referenced, ttlMs) {
	let names;
	try {
...
*/
function openPromptHistory(options) {
	const path = options.path ?? dshHomePath("history.jsonl");
	...
	const writeEntry = async (text) => {
		...
		if (text.length > MAX_INLINE_LENGTH) {
			...
			await writeFileAtomic(join(bodyDirectory, `${hash}.txt`), text, {
				mode: 384,
				dirMode: 448
			});
			record = {
	...
	const store = {
		path,
		load(limit = MAX_HISTORY_ENTRIES) {
			...
			for (let index = lines.length - 1; index >= 0; index--) {
				...
				if (text === void 0 || seen.has(text)) continue;
				seen.add(text);
		...
		append(text) {
			...
			if (isTruthy(process.env["DSH_SKIP_PROMPT_HISTORY"])) return;
			lastAppended = trimmed;
			queue = queue.then(async () => {
				await writeEntry(trimmed);
			}).catch((error) => {
	...
	try {
		if (statSync(path).size > COMPACT_THRESHOLD_BYTES) setTimeout(() => {
			compactPromptHistory(path).catch((error) => {
...
*/
const UPDATE_CHECK_PACKAGE_NAME = "deepseek-harness-tui";
...
*/
async function fetchLatestFromRegistry(name, signal) {
	...
	return typeof body.version === "string" && body.version !== "" ? body.version : void 0;
}
/**
...
async function readCache(path) {
	...
	if (typeof value !== "object" || value === null) return void 0;
	const record = value;
	if (typeof record.checkedAt !== "number" || !Number.isFinite(record.checkedAt)) return void 0;
	if (typeof record.latest !== "string" || record.latest === "") return void 0;
	return {
...
async function checkForUpdate(options) {
	try {
		...
		const cachePath = options.cachePath ?? dshHomePath("update-check.json");
		const cached = await readCache(cachePath);
		if (cached !== void 0) {
			const age = now() - cached.checkedAt;
			if (age >= 0 && age < ttlMs) return resultFor(options.currentVersion, cached.latest);
		}
		if (options.signal?.aborted === true) return void 0;
		const latest = await (options.fetchLatest ?? fetchLatestFromRegistry)(options.name, options.signal);
		if (latest === void 0 || latest.trim() === "") return void 0;
		await writeCache(cachePath, {
			checkedAt: now(),
			latest
		});
		return resultFor(options.currentVersion, latest);
	} catch (_offlineOrRefused) {
		return;
	}
}
/**
...
*/
function resultFor(currentVersion, latest) {
	return {
		latest,
...
*/
function updateCommandLine(name = UPDATE_CHECK_PACKAGE_NAME) {
		...
		const session = overlayManager.open({
			create: () => new PresetDialog(choices, current, defaultId, resolved.maxModelOptions, palette, (choice) => {
				...
				if (choice.broken !== void 0) {
					deps.appendNotice(t("preset.broken.refusal", {
...
//#endregion
//#region src/components/provider-dialog.ts
/**
...
var ProviderWizard = class {
	...
	busy = false;
	focused = false;
	constructor(initial, maxVisible, palette, cancel, redraw) {
		this.maxVisible = maxVisible;
		this.palette = palette;
		this.cancel = cancel;
		this.redraw = redraw;
	...
	*/
	...
	adoptStep(step) {
		...
		this.cursor = 0;
		if (step.kind === "select" && step.initial !== void 0) {
	...
	render(width) {
		...
		const innerWidth = Math.max(1, width - 4);
		const body = [];
		for (const line of step.lines ?? []) body.push(...wrapTextWithAnsi(this.palette.dim(displayText(line)), innerWidth));
	...
	renderText(step, innerWidth) {
		const lines = wrapTextWithAnsi(this.palette.text(displayText(step.prompt)), innerWidth);
		const shown = step.secret === true ? "•".repeat(Math.min(this.value.length, MAX_SECRET_DOTS)) : displayInlineText(this.value);
		const caret = this.focused && !this.busy ? this.palette.accent("▏") : "";
		lines.push(`${this.palette.dim("> ")}${this.palette.text(shown)}${caret}`);
		return lines;
	}
	renderList(step, innerWidth) {
		if (step.items.length === 0) return wrapTextWithAnsi(this.palette.dim(t("dialog.provider.empty")), innerWidth);
		const lines = [];
		...
		for (let index = start; index < end; index += 1) {
			...
			if (item.group !== void 0 && item.group !== printedGroup) {
		...
		if (end - start < step.items.length) lines.push(this.palette.dim(`  ${this.cursor + 1}/${step.items.length}`));
		return lines;
	}
	renderRow(step, item, active, innerWidth) {
		const marker = step.kind === "checklist" ? this.ticked.has(item.value) ? "[x] " : "[ ] " : "";
		const label = `${active ? "❯ " : "  "}${marker}${displayInlineText(item.label)}`;
		const styled = active ? this.palette.accent(label) : this.palette.text(label);
		if (item.description === void 0) return styled;
	...
	footer(step) {
		if (this.busy) return t("dialog.provider.hintBusy");
		if (step.kind === "text") return t("dialog.provider.hintText");
		if (step.kind === "checklist") return t("dialog.provider.hintChecklist");
		return t("dialog.provider.hintSelect");
	}
};
//#endregion
//#region src/chat/provider-store.ts
/**
...
function hostService(ctx, name) {
	...
	return service === null || service === void 0 ? void 0 : service;
}
/**
...
*/
function providerSettings(ctx) {
	const service = hostService(ctx, "settings");
	if (typeof service?.get !== "function" || typeof service.mutate !== "function") return void 0;
	return service;
}
/**
...
*/
function providerCredentials(ctx) {
	const service = hostService(ctx, "credentials");
	if (typeof service?.set !== "function") return void 0;
	return service;
}
/**
...
function maskApiKey(key) {
	const dots = "•".repeat(8);
	if (key.length <= 8) return dots;
	return `${dots}${key.slice(-4)}`;
}
/** Why a typed key cannot be stored, or `undefined` when it can. */
function apiKeyRefusal(key) {
...
function routeKeyRefusal(route) {
...
function credentialRefRefusal(ref) {
...
function defaultCredentialRef(provider) {
	...
	return `${CREDENTIAL_REF_PATTERN.test(upper) ? upper : `P_${upper}`}${CREDENTIAL_SUFFIX}`;
}
/**
...
*/
function storedProfiles(section) {
	...
	for (const [route, profile] of Object.entries(declared)) if (typeof profile === "object" && profile !== null) profiles.set(route, profile);
	return profiles;
}
...
function profileAt(section, path) {
	...
	return typeof cursor === "object" && cursor !== null ? cursor : void 0;
}
/**
...
*/
function readProviderRoster(ctx) {
	const llm = ctx.llm;
	...
	const sections = new Map([...namespaces].map((ns) => [ns, readSection(settings, ns)]));
	...
	for (const entry of directory) {
		...
		rows.set(entry.provider, {
			...
			...profile === void 0 ? {} : profileFacts(profile)
		});
	}
	for (const [ns, section] of sections) for (const [route, profile] of storedProfiles(section)) {
...
function defaultSettingsNamespace(ctx) {
	const llm = ctx.llm;
	return (typeof llm.listConfigurableProviders === "function" ? llm.listConfigurableProviders() : [])[0]?.settingsNs ?? FALLBACK_SETTINGS_NAMESPACE;
}
/**
...
*/
function namespaceAppliesLive(settings, ns) {
...
*/
async function saveProviderLogin(ctx, entry, key, draft = {}, credentialRef = entry.apiKeyEnv ?? defaultCredentialRef(entry.provider)) {
	const credentials = providerCredentials(ctx);
	if (credentials === void 0) throw new Error("this deployment has no credential store, so a key cannot be saved");
	const settings = providerSettings(ctx);
	if (settings === void 0) throw new Error("this deployment has no writable settings, so a provider cannot be saved");
...
//#endregion
//#region src/chat/provider-probe.ts
/**
...
function errorMessage(error) {
	...
	return typeof error === "string" && error !== "" ? error : t("provider.probe.silentFailure");
}
/**
...
*/
function isCredentialRefusal(message) {
...
*/
async function probeProviderKey(discover, target) {
	if (target.api !== void 0 && !LISTABLE_PROTOCOLS.has(target.api)) return {
		kind: "unsupported",
		message: t("provider.probe.unsupported", { api: target.api })
	};
	try {
		return {
			kind: "verified",
			models: await discover(target.settingsNs, {
				baseURL: target.baseURL,
				apiKey: target.apiKey,
				...target.api === void 0 ? {} : { api: target.api },
		...
		if (code === "DISCOVERY_UNSUPPORTED") return {
			kind: "unsupported",
			message
		};
		if (code === "INVALID_CREDENTIAL") return {
			kind: "rejected",
			message
		};
		if (isCredentialRefusal(message)) return {
			kind: "rejected",
			message
		};
		return {
...
*/
async function catalogModels(discover, settingsNs, provider, signal) {
	try {
...
//#endregion
//#region src/chat/login-command.ts
/**
...
function createLoginController(deps) {
	...
	const discover = (() => {
		const llm = ctx.llm;
		return typeof llm.discoverModels === "function" ? (ns, request) => llm.discoverModels(ns, request) : void 0;
	})();
	const closeFlow = () => {
		flowAbort?.abort();
		flowAbort = void 0;
	...
	*/
	const openWizard = (first) => {
		closeFlow();
		flowAbort = new AbortController();
		const session = overlayManager.open({
			create: () => {
				const created = new ProviderWizard(first, resolved.maxModelOptions, palette, closeFlow, () => {
					deps.requestRender();
				});
				wizard = created;
				return created;
	...
	*/
	...
	const reportSaved = (saved, key, verified) => {
	...
	const commit = async (entry, key, draft, verified, credentialRef) => {
	...
	const modelDraft = (models, chosen) => {
		return models.filter((model) => chosen.includes(model.id)).map((model) => ({
			id: model.id,
	...
	*/
	const runProbe = async (entry, key, baseURL, api) => {
		if (discover === void 0 || baseURL === void 0) return void 0;
	...
	const loginKeyStep = (entry) => ({
		kind: "text",
		title: t("login.key.title", { provider: entry.displayName }),
		lines: [entry.baseURL === void 0 ? t("login.key.catalogEndpoint") : t("login.key.endpoint", { url: entry.baseURL }), t("login.key.storage")],
		...
		onSubmit: (key) => {
			(async () => {
				...
				askSaveAnyway(outcome.message, () => {
					commit(entry, key, {}, false);
				});
			})();
		}
	});
	/** One roster row as a picker item, grouped by whether it is already configured. */
	const rosterItem = (entry) => ({
		value: entry.provider,
		label: entry.displayName === entry.provider ? entry.provider : `${entry.displayName} (${entry.provider})`,
		group: t(entry.configured ? "login.group.configured" : "login.group.available"),
		...entry.configured ? { description: entry.apiKeyEnv === void 0 ? t("login.roster.noKey") : t("login.roster.keyIn", { env: entry.apiKeyEnv }) } : {}
	});
	const handleLoginCommand = (raw) => {
		const roster = readProviderRoster(ctx);
		if (roster.length === 0) {
			deps.appendNotice(t("login.noProviders"), "warning");
			return;
		}
		if (providerCredentials(ctx) === void 0 || providerSettings(ctx) === void 0) {
	...
	const askModelByHand = (draft, entry, key) => {
		wizard?.setStep({
			kind: "text",
	...
	const askModelSelection = (draft, entry, key, models, verified) => {
		wizard?.setStep({
			kind: "checklist",
			title: t("provider.models.title"),
			lines: [plural(models.length, "provider.models.lead"), t("provider.models.tickNone")],
			items: models.map((model) => ({
				value: model.id,
				label: model.id,
				...model.name === void 0 || model.name === model.id ? {} : { description: model.name }
			})),
	...
	const askNewRouteKey = (draft, ns) => {
		...
		wizard?.setStep({
			...
			lines: [t("login.key.endpoint", { url: draft.baseURL }), t("provider.key.storedAs", { ref: draft.credentialRef })],
			prompt: t("login.key.prompt"),
			secret: true,
			refuse: apiKeyRefusal,
			onSubmit: (key) => {
				(async () => {
					...
					if (deps.isDisposed()) return;
					if (outcome === void 0 || outcome.kind === "aborted") {
						if (outcome === void 0) askModelByHand(draft, entry, key);
						return;
					}
					if (outcome.kind === "rejected") {
						wizard?.setRefusal(t("login.key.rejected", { message: outcome.message }));
					...
					askSaveAnyway(outcome.message, () => {
						askModelByHand(draft, entry, key);
					});
				})();
			}
		});
	...
	const askCredentialRef = (draft, ns) => {
		wizard?.setStep({
			kind: "text",
	...
	const askBaseUrl = (draft, ns) => {
		wizard?.setStep({
			...
			refuse: (value) => {
				...
				return ENDPOINT_PATTERN.test(value) ? void 0 : t("provider.endpoint.refuseShape");
			},
			onSubmit: (url) => {
				draft.baseURL = url;
	...
	const startProviderAdd = () => {
		...
		openWizard({
			...
			refuse: (value) => {
				...
				return taken.has(value) ? t("provider.add.taken", { value }) : void 0;
			},
			onSubmit: (route) => {
	...
	const listProviders = async () => {
		...
		if (configured.length > 0) {
			...
			for (const entry of configured) {
				const models = discover === void 0 ? [] : await catalogModels(discover, entry.settingsNs, entry.provider);
				const where = entry.apiKeyEnv === void 0 ? t("login.roster.noKey") : t("login.roster.keyIn", { env: entry.apiKeyEnv });
	...
	const handleProviderCommand = async (raw) => {
		const argument = raw.trim();
		if (argument === "") {
			await listProviders();
			return;
...
*/
function pendingUserQueue(inbox) {
	return [...inbox.nextStep.map((message) => ({
		message,
		placement: "steering"
	})), ...inbox.nextTurn.map((message) => ({
		message,
		placement: "queued"
...
*/
function queueItemPreview(message) {
...
*/
function isClipboardExportTarget(rawInput) {
...
*/
function clipSessionMarkdown(markdown, max = MARKDOWN_MAX_CHARS) {
	if (markdown.length <= max) return {
		text: markdown,
		truncated: false
	};
	...
	return {
		text: `${markdown.slice(0, kept)}${marker}`,
		truncated: true
	};
}
/**
...
function renderSessionMarkdown(entries, meta) {
	const blocks = [`# ${displayInlineText(meta.title === void 0 || meta.title.trim() === "" ? `${t("status.row.session")} ${meta.sessionId}` : meta.title)}`];
	const facts = [`- ${t("status.row.session")}: ${displayInlineText(meta.sessionId)}`];
	if (meta.cwd !== void 0) facts.push(`- ${t("status.row.directory")}: ${displayInlineText(meta.cwd)}`);
	if (meta.model !== void 0) facts.push(`- ${t("status.row.model")}: ${displayInlineText(meta.model)}`);
...
*/
function isInvalidTitleError(error) {
	return typeof error === "object" && error !== null && error.name === "SessionTitleInvalidError";
}
/**
...
*/
function isSupersededTitleError(error) {
...
*/
async function runRenameCommand(deps, session, rawInput, signal) {
	const titles = deps.titles;
	if (titles === void 0) return {
		kind: "error",
		text: t("rename.unavailable")
	};
	const typed = rawInput.trim();
	if (typed !== "") try {
		return {
			kind: "success",
			text: t("rename.done", { title: displayInlineText(titles.rename(session, typed).title) })
		};
	} catch (error) {
		if (isInvalidTitleError(error)) return {
			kind: "error",
			text: t("rename.invalid")
		};
		return {
			kind: "error",
			text: t("rename.failed", { error: displayInlineText(errorChain(error)) })
		};
	}
	deps.announceGenerating?.();
	try {
		const generated = await titles.refresh(session, signal);
		if (generated === void 0) return {
			kind: "error",
			text: t("rename.noContext")
		};
		return {
			kind: "success",
			text: t("rename.generated", { title: displayInlineText(generated.title) })
		};
	} catch (error) {
		if (signal.aborted || isSupersededTitleError(error)) return { kind: "success" };
		return {
			kind: "error",
			text: t("rename.generateFailed", { error: displayInlineText(errorChain(error)) })
		};
	}
}
/**
...
*/
function executableSuffixes(name, env, platform) {
	if (platform !== "win32") return [""];
	const extensions = (env["PATHEXT"] ?? DEFAULT_PATHEXT).split(";").map((extension) => extension.trim()).filter((extension) => extension.startsWith("."));
...
*/
function lookupOnPath(name, env, platform = process.platform) {
...
//#endregion
//#region src/chat/external-editor.ts
/**
...
const FALLBACK_EDITORS = process.platform === "win32" ? ["notepad"] : [
	"nano",
	"vim",
	"vi"
];
...
*/
function parseEditorCommandLine(value) {
	const parts = [];
	let current = "";
	let quote;
	let quoted = false;
...
*/
function resolveCommandWord(word, env) {
	if (isAbsolute(word) || word.includes("/") || word.includes(sep)) return isExecutableFile(word) ? word : void 0;
...
*/
function resolveExternalEditor(configured, env = process.env) {
	const candidates = [];
	if (configured !== void 0) {
		const setting = configured.trim();
		...
		if (visual !== "") candidates.push({
			line: visual,
			source: "visual"
		});
		else if (editor !== "") candidates.push({
			line: editor,
			source: "editor"
		});
		else for (const name of FALLBACK_EDITORS) candidates.push({
			line: name,
			source: "fallback"
		});
	}
	...
	for (const candidate of candidates) {
		...
		return {
			...
			editor: {
				command,
				args: [...(EDITOR_WAIT_FLAGS[name] ?? []).filter((flag) => !args.includes(flag)), ...args],
				name,
				source: candidate.source
			}
		};
	}
	return configured === void 0 ? { kind: "unset" } : {
		kind: "unresolved",
		command: firstWord ?? ""
	};
}
...
*/
function runEditor(editor, file, options = {}) {
	return new Promise((resolve, reject) => {
		const cwd = options.cwd !== void 0 && existsSync(options.cwd) ? options.cwd : process.cwd();
...
*/
async function editTextExternally(text, editor, options = {}) {
	...
	try {
		...
		return {
			kind: "edited",
			text: trimOneTrailingNewline(await readFile(file, "utf8"))
		};
	} catch (error) {
		return {
			kind: "failed",
...
async function presetArgumentCompletions(source, argumentPrefix, limit) {
	...
	if (!copying && matches("copy", typed)) items.push({
		value: "copy ",
		label: "copy",
		description: t("preset.arg.copy")
	});
	for (const preset of presets) {
		if (!matches(preset.id, typed)) continue;
		const detail = preset.broken === void 0 ? preset.description ?? preset.name : t("preset.broken.badge", { reason: preset.broken });
		const description = [preset.id === source.defaultId ? t("preset.badge.default") : void 0, detail].filter((part) => part !== void 0 && part !== "").join(" · ");
		items.push({
...
*/
function loginArgumentCompletions(roster, argumentPrefix, limit) {
	const typed = argumentPrefix.trim();
	const items = [];
	for (const entry of roster) {
		if (!matches(entry.provider, typed) && !matches(entry.displayName, typed)) continue;
		const state = !entry.configured ? t("login.group.available") : entry.apiKeyEnv === void 0 ? t("login.roster.noKey") : t("login.roster.keyIn", { env: entry.apiKeyEnv });
		const description = [entry.displayName === entry.provider ? void 0 : displayInlineText(entry.displayName), state].filter((part) => part !== void 0 && part !== "").join(" · ");
		items.push({
			value: entry.provider,
			label: itemLabel(entry.provider),
			description
		});
		if (items.length >= limit) break;
	}
...
*/
function providerArgumentCompletions(argumentPrefix) {
	return matches("add", argumentPrefix.trim()) ? [{
		value: "add",
		label: "add",
		description: t("provider.arg.add")
	}] : null;
}
...
*/
function copyArgumentCompletions(answers, argumentPrefix, limit) {
	const typed = argumentPrefix.trim();
	const items = [];
...
*/
function exportArgumentCompletions(argumentPrefix) {
	return matches("clipboard", argumentPrefix.trim()) ? [{
		value: "clipboard",
		label: "clipboard",
		description: t("export.arg.clipboard")
	}] : null;
}
/**
...
*/
const EXTERNAL_EDITOR_HINT_MS = 5e3;
/**
...
*/
const JOBS_ELAPSED_TICK_MS = 1e3;
/**
...
function createTuiChat(ctx, config, runtime) {
	...
	*/
	let busyEnter = storedPreferences.busyEnter;
	...
	*/
	const attachedContexts = /* @__PURE__ */ new Map();
	/**
	...
	*/
	const compactionEngine = () => (typeof presetRoster?.serviceFor === "function" ? presetRoster.serviceFor(agent, "compaction") : void 0) ?? ctx.get("compaction");
	...
	* @returns the editor, or why there is none.
	*/
	const externalEditor = () => resolveExternalEditor(resolved.externalEditor);
	/**
	...
	*/
	...
	const header = new HeaderComponent({
		...
		skills: headerSkills,
		plugins: () => {
			if (headerPlugins === void 0) {
				const entries = ctx.get("pluginInventory")?.list().entries;
				if (entries !== void 0) headerPlugins = [...new Set(entries.filter((entry) => entry.enabled).map((entry) => entry.moduleName))].sort((left, right) => left.localeCompare(right));
			}
	...
	*/
	let liveJobs = jobs === void 0 ? 0 : jobCounts(jobs.list(agent)).live;
	const promptValues = [
	...
	const [cwdValue, gitValue, tokenValue, modelValue, contextValue, goalValue, queuedValue, jobsValue, symbolValue, indicatorValue] = promptValues;
	/* v8 ignore next -- the fixed built-in registration list always supplies each handle. */
	if (cwdValue === void 0 || gitValue === void 0 || tokenValue === void 0 || modelValue === void 0 || contextValue === void 0 || goalValue === void 0 || queuedValue === void 0 || jobsValue === void 0 || symbolValue === void 0 || indicatorValue === void 0) throw new Error("TUI prompt built-ins failed to initialize");
	const updatePromptValues = () => {
		...
		contextValue.set(pressure === void 0 ? void 0 : `  ${pressure.level === "normal" ? palette.dim(t("prompt.context", { percent: pressure.percentUsed })) : (pressure.level === "critical" ? palette.error : palette.warning)(t("prompt.contextLow", { remaining: pressure.percentRemaining }))}`);
		if (pressure !== void 0) announceContextPressure(pressure);
		const goalFragment = formatGoalPrompt(goalState.goal);
		goalValue.set(goalFragment === void 0 ? void 0 : `  ${palette.dim(goalFragment)}`);
		const queued = runningStatus === void 0 ? void 0 : formatQueuedStatus(queueCount);
		queuedValue.set(queued === void 0 ? void 0 : palette.dim(queued));
		jobsValue.set(liveJobs === 0 ? void 0 : `  ${palette.dim(plural(liveJobs, "prompt.jobs"))}`);
		symbolValue.set(palette.bold(palette.accent("dsh")));
		statusLine.setText(compacting !== void 0 ? palette.dim(t(compactCancelling ? "prompt.compactingCancelling" : "prompt.compacting", { duration: formatStatusDuration(renderTime - compacting.startedAt) })) : flashingStatus === void 0 ? "" : palette.dim(displayText(flashingStatus.text)));
	...
	*/
	const watchJobs = () => {
		...
		disposeJobChanges?.();
		disposeJobChanges = void 0;
		jobs = registry;
		if (registry !== void 0) disposeJobChanges = registry.onJobsChanged(readJobs);
		readJobs();
	};
	const disposeJobsService = ctx.on("internal/service", (name) => {
		if (disposed) return;
	...
	*/
	const canCompact = () => compactionEngine() !== void 0;
	/**
	...
	*/
	const announceContextPressure = (pressure) => {
		if (compacting !== void 0) return;
		...
		queueMicrotask(() => {
			if (disposed) return;
			const action = t(canCompact() ? agent.status === "idle" ? "notice.contextCompactAction" : "notice.contextCompactActionAfterTurn" : "notice.contextNoCompactAction");
			appendNotice(t(announce === "critical" ? "notice.contextCritical" : "notice.contextLow", {
				remaining,
				used,
				capacity,
				action
	...
	*/
	const clipboardConfirmation = (path, count) => path === "native" ? t("status.flash.copied", { count }) : t(path === "tmux-buffer" ? "status.flash.copiedTmux" : "status.flash.copiedOsc52");
	/**
	...
	*/
	const copyAnswer = (request) => {
		if (request.kind === "invalid") return {
			kind: "error",
			text: t("copy.usage", { input: displayInlineText(request.input) })
		};
		...
		if (answers.length === 0) {
			...
			return {
				kind: "error",
				text: t("status.flash.nothingToCopy")
			};
		}
		const index = request.kind === "latest" ? 0 : request.n - 1;
		if (index >= answers.length) return {
			kind: "error",
			text: plural(answers.length, "copy.outOfRange")
		};
		...
		copyToClipboard(text).then((sequence) => {
			...
			flashStatus(request.kind === "latest" ? detail : t("status.flash.copiedNth", {
				n: index + 1,
				total: answers.length,
				detail
			}));
	...
	const overlayManager = new TuiOverlayManager({
		...
		show: (component, options, placement) => {
			...
			return { hide() {
				questionContainer.clear();
				if (inlineFocusTarget === component) inlineFocusTarget = void 0;
				ui.setFocus(editor);
			} };
		},
	...
	const disposeChildRouteFallback = ctx.on("agent/request", async (_payload, next) => {
		const resolved = await next();
		if (resolved.provider !== void 0 && resolved.provider !== "" && resolved.model !== void 0 && resolved.model !== "") return resolved;
		...
		return {
			...
			provider: selected.provider,
			model: selected.model,
			...resolved.reasoningEffort === void 0 && selected.reasoningEffort !== void 0 ? { reasoningEffort: selected.reasoningEffort } : {}
		};
	});
	...
	* reaches the limit.
	*/
	const refreshEditorHint = () => {
		const teaching = runningStatus !== void 0 && queueTeachable && queueCount > 0 && (queueHintCounted || queueUpHintSeen < 3);
		if (teaching && !queueHintCounted && editor.getText() === "") {
			queueHintCounted = true;
			queueUpHintSeen += 1;
			preferences.save({ queueUpHintSeen });
		}
		editor.hint = teaching ? palette.dim(t("prompt.queuedEditHint")) : runningStatus !== void 0 ? palette.dim(displayInlineText(resolved.theme.inputPlaceholder)) : void 0;
	};
	/**
	...
	*/
	const markQueueHintLearned = () => {
		queueHintCounted = false;
		if (queueUpHintSeen < 3) {
			queueUpHintSeen = 3;
			preferences.save({ queueUpHintSeen });
		}
		refreshEditorHint();
	...
	*/
	const refreshQueueState = () => {
		...
		queueTeachable = queueCount > 0 && runningStatus !== void 0;
	...
	* when the turn ends.
	*/
	const prefixGrantedCallIds = /* @__PURE__ */ new Set();
	/**
	...
	*/
	const presentApprovalCall = (callId) => {
		...
		if (definition?.presentCall === void 0) return void 0;
		try {
			const view = definition.presentCall(args);
			if (view?.card === "terminal") return {
				command: view.title,
				...view.cwd === void 0 ? {} : { cwd: view.cwd }
			};
	...
	*/
	const disposeApprovals = ctx.on("approval/request", (req, next) => {
		...
		const matchContext = {
			...access === void 0 ? {} : { access },
			...presented?.cwd === void 0 ? {} : { cwd: presented.cwd }
		};
		if (sessionApprovals.has(req.toolName) || approvalRules.matchesTool(req.toolName, matchContext)) return Promise.resolve("allowed-once");
		if (presented?.command !== void 0 && req.callId !== void 0 && !prefixGrantedCallIds.has(req.callId) && approvalRules.matchesCommand(req.toolName, presented.command, matchContext)) {
		...
		return new Promise((resolveOutcome) => {
			...
			overlay = overlayManager.open({
				...
				create: () => new ApprovalDialog({
					...
					...presented?.command === void 0 ? {} : { command: presented.command },
					...presented?.cwd === void 0 || isInsideProject(presented.cwd, cwd) ? {} : { commandCwd: presented.cwd },
					...presented?.diffs === void 0 ? {} : { diffs: presented.diffs },
					...access === void 0 ? {} : { access }
				}, palette, (decision) => {
					...
					if (decision.outcome === "allowed-once") {
						...
						if (grant.prefix === void 0) {
							approvalRules.allow(scope);
							appendNotice(access === void 0 ? t("approval.grantedProject", {
								tool,
								path
							}) : t("approval.grantedProjectAccess", {
								tool,
								access: displayText(access),
								path
							}));
							return;
						}
						approvalRules.allow({
							...scope,
							content: grant.prefix
						});
						appendNotice(t("approval.grantedPrefix", {
							rule: serializeApprovalRule({
								...scope,
								content: grant.prefix
							}),
							path
	...
	*/
	...
	} catch (_terminalWillNotTakeTheQuestion) {}
	/**
	...
	* @param options - `persist` writes it to the settings document as well.
	*/
	const setBusyEnter = (next, options) => {
		busyEnter = next;
	...
	*/
	const showDebug = () => {
		...
		showPanel("debug (shift+ctrl+d)", [
			...
			`overlay ${overlayManager.hasActiveOverlay() ? "active" : "none"} · pending steering ${String(pendingSteering.size)} · inbox queue ${String(queueCount)}`,
	...
	*/
	const settingsEntries = () => [
		...
		},
		{
			...
			value: () => busyEnter,
			format: (behavior) => t(`settings.busyEnter.${behavior}`),
			set: (next) => {
	...
	const showSubagents = () => {
		...
		*/
		const read = () => {
			issued += 1;
			const seq = issued;
			const current = () => !disposed && !scan.signal.aborted && seq > answered;
			directory.listDescendants(agent.session.id, scan.signal).then((entries) => {
				if (!current()) return;
				answered = seq;
				scanned = [...entries];
		...
		const session = overlayManager.open({
			create: () => {
				panel = new SubagentsPanel(scanned, panelRows, palette, () => {
					session.close();
				});
				return panel;
			},
		...
		let refreshTimer;
		const refresh = () => {
			if (refreshTimer !== void 0 || scan.signal.aborted) return;
	...
	*/
	const readSubagentCounts = async (signal) => {
		const directory = subagentDirectory(ctx);
		if (directory === void 0) return void 0;
		return directory.listDescendants(agent.session.id, signal).then(subagentCounts, () => void 0);
	};
	/**
	...
	*/
	const showJobs = (registry) => {
		...
		const retime = () => {
			if (!rows.some((row) => row.status === "running" || row.status === "stopping")) {
				stopTicking();
				return;
			}
			if (ticker !== void 0) return;
		...
		const session = overlayManager.open({
			create: () => {
				panel = new JobsPanel(rows, panelRows, palette, now, () => {
					session.close();
				});
				return panel;
			},
		...
		session.closed.then(() => {
			stopTicking();
			if (refreshJobsPanel === refresh) refreshJobsPanel = void 0;
			if (panelOverlay === session) panelOverlay = void 0;
		});
		retime();
		requestRender();
	...
	*/
	...
	const showStatus = async (signal) => {
		...
		const [assembly, subagents] = await Promise.all([ctx.systemPrompt.assemble(assembleContextFor(agent, signal)), readSubagentCounts(signal)]).finally(settleHint);
		...
		if (pressure !== void 0) context = t("status.contextValue", {
		...
		const groups = [
			...
			[
				...
				...stats === void 0 ? [] : [[t("status.row.sessionTotals"), formatSessionStats(stats)]],
				...subagents === void 0 ? [] : [[t("status.row.subagents"), t("status.subagentsValue", { ...subagents })]],
				...backgroundJobs === void 0 ? [] : [[t("status.row.jobs"), t("status.jobsValue", { ...jobCounts(backgroundJobs) })]]
			],
			[
		...
		showPanel("/status", [
			...
			...queue.length === 0 ? [] : [
				...
				...queue.map((item, index) => `${String(index + 1)}. ${palette.dim(`[${t(item.placement === "steering" ? "status.queued.steering" : "status.queued.nextTurn")}]`)} ${displayInlineText(queueItemPreview(item.message))}`)
			],
			"",
	...
	*/
	const argumentCompletionsFor = (name) => {
		switch (name) {
			...
			case "login": return (prefix) => loginArgumentCompletions(readProviderRoster(ctx), prefix, resolved.maxModelOptions);
			case "provider": return providerArgumentCompletions;
			case "copy": return (prefix) => copyArgumentCompletions(collectAnswerTexts(store.getSnapshot().nodes), prefix, resolved.maxModelOptions);
			case "export": return exportArgumentCompletions;
	...
	*   carried them.
	*/
	const exportSessionMarkdown = async (signal) => {
		const snapshot = store.getSnapshot();
		const entries = transcriptEntries(snapshot.nodes);
		if (entries.length === 0) return {
			kind: "error",
			text: t("export.clipboard.empty")
		};
		const path = clipboardPath();
		...
		const document = path === "osc52" ? clipSessionMarkdown(markdown) : {
			text: markdown,
			truncated: false
		};
		try {
			...
			return {
				kind: "success",
				text: plural(entries.length, document.truncated ? "export.clipboard.truncated" : "export.clipboard.done", {
			...
			return {
				kind: "error",
				text: t("notice.copyFailed", { error: errorChain(error) })
			};
		}
	};
	/**
	...
	const commandFiber = agent.ctx.inject(["commands"], (commandCtx) => {
		...
		commandCtx.commands.register({
			...
			handler: ({ rawInput, signal }) => runRenameCommand({
				...
				announceGenerating: () => {
					appendNotice(t("rename.generating"));
				}
		...
		commandCtx.commands.register({
			...
			handler: async (invocation) => {
				if (activeCompaction !== void 0) return {
					kind: "error",
					text: t("compact.inFlight")
				};
				compactCancelling = false;
				const controller = new AbortController();
				const relay = () => {
					controller.abort(/* @__PURE__ */ new Error("compaction cancelled by the user"));
				};
				...
				try {
					return await runCompactCommand({
						...
						agent,
						expandKey: () => keyLabel(keybindings, "app.tools.cycle")
					}, invocation.rawInput, controller.signal, invocation.commandId);
				} finally {
					invocation.signal.removeEventListener("abort", relay);
					activeCompaction = void 0;
					if (compacting === void 0) compactCancelling = false;
				}
			}
		});
		commandCtx.commands.register({
		...
		commandCtx.commands.register({
			...
			handler: ({ rawInput, signal }) => isClipboardExportTarget(rawInput) ? exportSessionMarkdown(signal) : exportSessionLog({
	...
	*/
	const dispatchMessage = (content, attachedContext, gesture = "enter") => {
	...
	*/
	const settleAttachedContext = (id, claimed) => {
	...
	*/
	const deliverSkill = (skill, instructions) => {
		...
		dispatchMessage([{
			type: "text",
			text: renderSkillEcho(skill.name, instructions)
		}], context);
	};
	/**
	...
	*/
	...
	const submitLine = (raw) => {
		...
		const command = parseCommand(text) !== void 0;
	...
	*/
	editor.onChange = (text) => {
		...
		if (flashingStatus !== void 0 || compacting !== void 0) return;
	...
	*/
	const openExternalEditor = async () => {
		...
		try {
			...
			if (result.kind === "exit") {
				appendNotice(t("notice.externalEditorExit", {
					editor: editorSpec.name,
					code: result.code
				}), "warning");
				return;
			}
	...
	*/
	const handleEscape = () => {
		...
		if (activeCompaction !== void 0) {
			disarmEscape();
			compactCancelling = true;
			activeCompaction.abort(/* @__PURE__ */ new Error("compaction cancelled by the user"));
			if (compacting === void 0) flashStatus(t("status.flash.compactCancelling"));
			requestRender();
			return;
		}
		if (editor.getText() !== "") {
	...
	const removeInputListener = ui.addInputListener((data) => {
		...
		if (keybindings.matches(data, "tui.editor.cursorUp")) {
			if (!press) return upClaimed ? { consume: true } : void 0;
			upClaimed = runningStatus !== void 0 && inlineFocusTarget === void 0 && !editor.isShowingAutocomplete() && editor.getText() === "" && popQueuedForEdit();
			return upClaimed ? { consume: true } : void 0;
		}
	...
	*/
	const disposeAttachedContext = ctx.on("agent/pre-step", async (payload, next) => {
		...
		for (const message of payload.messages) {
			...
			if (attached === void 0 || attached.injected) continue;
			attachedContexts.delete(message.id);
			parked.push(attached.message);
		}
		return parked.length === 0 ? decision : {
			...decision,
			messages: [...parked, ...decision.messages]
		};
	});
	...
	const replayed = [];
	for (const node of initial.nodes) if (node.kind === "user-message" && node.source === "user") replayed.push(node.text.trim());
	...
	editor.seedHistory([...fromSession, ...fromDisk].slice(0, 100));
	...
	if (resolved.updateCheck && runningVersion !== void 0) checkForUpdate({
		...
		(update) => {
			...
			if (update === void 0 || !update.hasUpdate) return;
...
*/
async function attachSessionToWorkspace(ctx, sessionId) {
...
async function openStartupAgent(ctx, startup, agentOptions) {
	...
	if (identity !== void 0) {
		if (identity.resume) return ctx.agents.resume(await resumeOptions(identity.id));
...
/* v8 ignore stop */
//#endregion
export { Config, DEFAULT_FILE_SEARCH_EXCLUDED_DIRECTORIES, DEFAULT_FILE_SEARCH_MAX_ENTRIES, DEFAULT_FILE_SEARCH_MAX_RESULTS, FILE_REFERENCE_PROMPT, INITIAL_SKILL_KEY, MAIN_SESSION_ID_KEY, TUI_GOODBYE_MESSAGE_KEY, TuiConfigSchema, TuiExtensionService, TuiPromptService, apply, createTuiChat, disposeRootAndExit, inject, mountTui, name, openStartupAgent, renderSkillEcho, resolveTuiConfig, startupFailureMessage, startupRefusal };
