...
*/
function storedProfiles(settings, ns) {
	const profiles = /* @__PURE__ */ new Map();
	if (settings === void 0) return profiles;
	let section;
	try {
...
function readProviderRoster(ctx) {
	...
	const sections = new Map([...namespaces].map((ns) => [ns, storedProfiles(settings, ns)]));
	const rows = /* @__PURE__ */ new Map();
	const add = (entry) => {
		const existing = rows.get(entry.provider);
		if (existing !== void 0 && existing.configured && !entry.configured) return;
	...
	for (const [ns, profiles] of sections) for (const [route, profile] of profiles) add({
		provider: route,
