...
function formatCollapseHint(hint, displayPath) {
	const text = hint.kind === "path" ? displayPath(hint.value) : hint.kind === "pattern" ? `"${hint.value}"` : `$ ${hint.value}`;
...
*/
function planModeRow(palette, scheme = "dark") {
...
*/
function collapsedSummary(group) {
	const parts = [];
...
var CollapsedGroupComponent = class extends CachedCardComponent {
	...
	*/
	constructor(group, palette, displayPath, expandKey) {
		super();
		this.group = group;
		this.palette = palette;
	...
	*/
	...
	renderLines(width) {
		...
		if (group.active && group.hint !== void 0) {
			const inner = Math.max(20, width - RESULT_PREFIX_WIDTH);
...
var TranscriptReconciler = class {
	...
	*/
	groupView(group) {
		...
		const component = new CollapsedGroupComponent(group, this.deps.palette, (path) => displayPath(path, this.deps.cwd), this.deps.expandKey);
		this.views.set(key, {
			kind: "group",
			signature,
			component
	...
	*/
	toolView(node) {
		...
		const component = new ToolCardComponent(node.name, node.args, this.deps.toolDefinition(node.name), this.deps.maxToolOutputLines, this.deps.maxDiffEditLength, this.deps.palette, this.deps.mdTheme);
...
function foldEvent(nodes, event) {
	...
	switch (event.type) {
		...
		case "tool/call": {
			...
			if (assistant !== void 0 && !assistant.toolCalls.includes(callId)) {
